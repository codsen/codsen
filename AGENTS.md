...
## Package README changes warrant npm releases

A package-root `README.md` is part of the published product: it is the npm
page's documentation and marketing copy. Changes generated by `lect`, including
dependency badges or notices, must make the package eligible for the next
normal release even when its runtime code is unchanged. Patch releases for
README updates are intentional. Do not restore a blanket Markdown or README
exclusion in `lerna.json` to reduce release noise.

- `lerna.json#ignoreChanges` excludes `AGENTS.md` and `CHANGELOG.md` explicitly;
  other package Markdown is eligible. The repository-root README is outside
  the workspaces and does not select a package.
- Use package-scoped `docs(<package>): ...` commits for README-only changes.
  These commits are release-bearing under this policy, so the per-package
  commit rule above applies. Keep a shared generator change and additions to
  `@codsen/data` in their own commits. Never put a package's README in another
  package's `feat` or breaking-change commit: Lerna reads commit types from
  each package's path history, not only from the subject's scope.
- The installed conventional-commit calculator recommends a patch for a
  README-only `docs` history. Other unreleased features or breaking changes
  can require a larger bump. Lerna can also select local dependents, so a
  shared README update may produce a broad release batch. This cost is accepted
  to keep npm pages current; batch copy edits before release preparation.
- Keep the generated README deterministic and edit its `lect` inputs. An
  unchanged generator result must not create a release candidate merely
  because generation ran again.
- Release eligibility does not publish anything immediately. The normal
  reviewed release proposal and protected publishing workflow still apply.
...
When present, the monorepo-wide review documents in the local `logs/` directory
record audited architectural, CI, release, testing, and package-quality
improvements. Read them when the task concerns this backlog or the user asks to
continue the monorepo review plan. Treat them as a backlog, not as authority to
expand the current task. Revalidate each finding against the current tree before
implementation, and update the item's status and evidence when work is
completed.

When a user references an item by a `REV-*` identifier, search the files in
`logs/` for that identifier before planning or making changes.
...
repository-root `logs/` directory. Use ordinary Markdown with a descriptive
title and natural sections, and follow an existing `logs/` document as the
...
- Dependency markers are derived by `ops/helpers/dependencyStatuses.js` from
  current package manifests. Do not maintain package flags or website lists by
