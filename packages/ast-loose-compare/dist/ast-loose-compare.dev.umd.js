...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.astLooseCompare = {}));
...
function internalCompare(bigObj, smallObj, res) {
  ...
  if (Array.isArray(bigObj) && Array.isArray(smallObj)) {
    if (smallObj.length > 0) {
      for (i = 0, len = smallObj.length; i < len; i++) {
        if (Array.isArray(smallObj[i]) || lodash_isplainobject(smallObj[i])) {
  ...
  } else if (lodash_isplainobject(bigObj) && lodash_isplainobject(smallObj)) {
    ...
    if (Object.keys(smallObj).length > 0) {
      ...
      for (i = 0, len = keysArr.length; i < len; i++) {
        ...
        if (Array.isArray(smallObj[keysArr[i]]) || lodash_isplainobject(smallObj[keysArr[i]]) || typeof smallObj[keysArr[i]] === "string") {
