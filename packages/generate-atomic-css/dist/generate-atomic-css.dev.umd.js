...
function extractConfig(str) {
    ...
    if (str.includes(CONFIGHEAD) && str.includes(CONFIGTAIL)) {
    ...
    else if (str.includes(CONFIGHEAD) &&
        ...
        if (extractedConfig.includes(CONTENTHEAD)) {
            ...
            if (rightSeq(extractedConfig, sliceFrom - 1, "*", "/")) {
                sliceFrom =
                    rightSeq(extractedConfig, sliceFrom - 1, "*", "/")
                        .rightmostChar + 1;
            }
            let sliceTo = null;
            if (str.includes(CONTENTTAIL)) {
                ...
                if (str[right(str, contentAfterStartsAt - 1)] === "*" &&
        ...
        // remove content tail
            ...
            if (leftSeq(extractedConfig, sliceTo, "/", "*")) {
                sliceTo = leftSeq(extractedConfig, sliceTo, "/", "*")
                    .leftmostChar;
            }
            ...
            if (right(str, str.indexOf(CONTENTTAIL) + CONTENTTAIL.length)) {
                ...
                if (str[right(str, contentAfterStartsAt)] === "*" &&
                    str[right(str, right(str, contentAfterStartsAt))] === "/") {
                    contentAfterStartsAt =
                        right(str, right(str, contentAfterStartsAt)) +
                            1;
                    if (right(str, contentAfterStartsAt)) {
                        rawContentBelow = str.slice(contentAfterStartsAt);
                    }
            ...
            if (source[y] === "$" && source[y - 1] === "$" && source[y - 2] === "$") {
                ...
                if (i === 0 &&
                    ...
                    if (!source[y - 3].trim().length ||
                        ...
                        padLeftIfTheresOnTheLeft.some((val) => source
                        ...
                        res += `${source.slice(startPoint, y - 2)}${pad
                            ? String(i).padEnd(String(to).length +
                                (i === 0 && unitThatFollow
                                    ? unitThatFollow.length
                                    : 0))
                            : i}`;
...
function prepConfig(str, progressFn, progressFrom, progressTo, trim = true, generatedCount, pad) {
    ...
    return trimBlankLinesFromLinesArray(str
        .split(/\r?\n/)
        .map((rowStr, i, arr) => rowStr.includes("$$$")
        ? prepLine(rowStr, progressFn, progressFrom + ((progressTo - progressFrom) / arr.length) * i, progressFrom +
            ((progressTo - progressFrom) / arr.length) * (i + 1), generatedCount, pad)
        : bump(rowStr, generatedCount)), trim).join("\n");
}
...
function genAtomic(str, originalOpts) {
    ...
    if (str.includes(CONFIGTAIL) &&
        ...
        if (str[right(str, str.indexOf(CONFIGTAIL) + CONFIGTAIL.length)] ===
            "*" &&
            str[right(str, right(str, str.indexOf(CONFIGTAIL) + CONFIGTAIL.length))] === "/") {
            sliceFrom =
        ...
        if (str.slice(right(str, sliceFrom - 1)).startsWith(CONTENTHEAD)) {
            ...
            if (str[right(str, sliceFrom - 1)] === "*" &&
                str[right(str, right(str, sliceFrom - 1))] === "/") {
                sliceFrom =
                    right(str, right(str, sliceFrom - 1)) + 1;
            }
            ...
            if (str.includes(CONTENTTAIL)) {
                ...
                if (str[right(str, sliceFrom)] === "*" &&
                    str[right(str, right(str, sliceFrom))] === "/") {
                    sliceFrom =
                        right(str, right(str, sliceFrom)) + 1;
                }
            }
        }
    ...
    if (typeof rawContentBelow === "string") {
        ...
        if (rawContentBelow.trim().endsWith("/*") &&
