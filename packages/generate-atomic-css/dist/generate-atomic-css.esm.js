    ...
    if (extractedConfig.includes(CONTENTHEAD)) {
      ...
      if (str.includes(CONTENTTAIL)) {
        ...
        if (str[right(str, contentAfterStartsAt - 1)] === "*" && str[right(str, right(str, contentAfterStartsAt - 1))] === "/") {
          contentAfterStartsAt = right(str, right(str, contentAfterStartsAt - 1)) + 1;
        }
...
function genAtomic(str, originalOpts) {
  ...
  const finalRes = `${trimIfNeeded(`${frontPart}${prepConfig(extractedConfig, opts.reportProgressFunc, opts.reportProgressFuncFrom, opts.reportProgressFuncTo, true,
