...
function rMerge(arrOfRanges, originalOpts) {
  ...
  if (originalOpts) {
    if (isObj(originalOpts)) {
      opts = _objectSpread2(_objectSpread2({}, defaults$5), originalOpts); // 1. validate opts.progressFn
    ...
    opts = _objectSpread2({}, defaults$5);
...
function patcher(str, generalOpts) {
  ...
  var opts = _objectSpread2(_objectSpread2({}, defaults$7), generalOpts);
  ...
  traverse(cparser(str), function (token, _val, innerObj) {
    /* istanbul ignore else */
    if (isObj$4(token) && token.type === "comment" && !knownCommentTokenPaths.some(function (oneOfRecordedPaths) {
    ...
    isObj$4(token) && // filter by type and tag name
  ...
  if (ranges.current()) {
    var result = rApply(str, ranges.current());
    ranges.wipe();
    return {
      result: result
    };
