...
t.test(
  `01.01 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onmouseup, error level 0`,
  t => {
...
t.test(
  `01.02 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onmouseup, error level 1`,
  t => {
    const str = `<input class="z">`;
    const linter = new Linter();
    const messages = linter.verify(str, {
...
t.test(
  `01.03 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onmouseup, error level 2`,
  t => {
    const str = `<input class="z">`;
    const linter = new Linter();
    const messages = linter.verify(str, {
...
t.test(
  ...
  t => {
    ...
    t.equal(applyFixes(str, messages), `<input onmouseup="something">`);
    t.match(messages, [
      {
        ruleId: "attribute-validate-onmouseup",
        idxFrom: 18,
