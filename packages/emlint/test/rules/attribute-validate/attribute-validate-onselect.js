...
tap.test(
  `01 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onselect, error level 0`,
  (t) => {
...
tap.test(
  `02 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onselect, error level 1`,
  (t) => {
    const str = `<input class="z">`;
    const linter = new Linter();
    const messages = linter.verify(str, {
...
tap.test(
  `03 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no onselect, error level 2`,
  (t) => {
    const str = `<input class="z">`;
    const linter = new Linter();
    const messages = linter.verify(str, {
...
tap.test(
  ...
  (t) => {
    ...
    t.equal(applyFixes(str, messages), `<input onselect="something">`, "08.01");
    t.match(
      messages,
      [
        {
