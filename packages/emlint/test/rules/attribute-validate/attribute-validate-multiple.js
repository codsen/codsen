...
t.test(
  `01.01 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no multiple, error level 0`,
  (t) => {
...
t.test(
  `01.02 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no multiple, error level 1`,
  (t) => {
...
t.test(
  `01.03 - ${`\u001b[${34}m${`validation`}\u001b[${39}m`} - no multiple, error level 2`,
  (t) => {
    const str = `<select><img>`;
    const linter = new Linter();
    const messages = linter.verify(str, {
...
t.test(
  ...
  (t) => {
    ...
    // can fix:
    t.equal(applyFixes(str, messages), `<select multiple>`);
    t.match(messages, [
      {
        ruleId: "attribute-validate-multiple",
        idxFrom: 16,
...
t.test(
  `04.01 - ${`\u001b[${34}m${`XHTML`}\u001b[${39}m`} - healthy multiple checkbox, as HTML`,
  (t) => {
    const str = `<select multiple>`;
    const linter = new Linter();
    const messages = linter.verify(str, {
    ...
    // can fix:
    t.equal(applyFixes(str, messages), `<select multiple="multiple">`);
    t.match(messages, [
      {
        ruleId: "attribute-validate-multiple",
        idxFrom: 8,
...
t.test(
  ...
  (t) => {
    ...
    t.match(messages[0].fix.ranges, [[16, 19, `="multiple"`]]);
    t.equal(applyFixes(str, messages), `<select multiple="multiple">`);
    t.end();
  }
);
