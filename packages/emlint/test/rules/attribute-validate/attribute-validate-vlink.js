...
test(`01 - ${`\u001b[${34}m${"validation"}\u001b[${39}m`} - no vlink, error level 0`, () => {
  let str = "<body>";
...
test(`02 - ${`\u001b[${34}m${"validation"}\u001b[${39}m`} - no vlink, error level 1`, () => {
  let str = "<body>";
  let linter = new Linter();
  let messages = linter.verify(str, {
    rules: {
...
test(`03 - ${`\u001b[${34}m${"validation"}\u001b[${39}m`} - no vlink, error level 2`, () => {
  let str = "<body>";
  let linter = new Linter();
  let messages = linter.verify(str, {
    rules: {
...
test(`04 - ${`\u001b[${34}m${"validation"}\u001b[${39}m`} - healthy vlink`, () => {
  let str = "<body class='zz' vlink='#CCCCCC' id='yy aa'>"; // <-- notice single quotes
  let linter = new Linter();
  let messages = linter.verify(str, {
    rules: {
      "attribute-validate-vlink": 2,
...
test(`05 - ${`\u001b[${36}m${"whitespace"}\u001b[${39}m`} - space in front`, () => {
  ...
  equal(applyFixes(str, messages), '<body vlink="#CCCCCC">', "05.01");
  compare(ok, messages, [
    {
...
test(`06 - ${`\u001b[${36}m${"whitespace"}\u001b[${39}m`} - space after`, () => {
  ...
  equal(applyFixes(str, messages), '<body vlink="#CCCCCC">', "06.01");
  compare(ok, messages, [
    {
      ruleId: "attribute-validate-vlink",
      idxFrom: 20,
...
test(`07 - ${`\u001b[${36}m${"whitespace"}\u001b[${39}m`} - copious whitespace around - 6 digit hex`, () => {
  ...
  equal(applyFixes(str, messages), '<body vlink="#CCCCCC">', "07.01");
  compare(ok, messages, [
    {
      ruleId: "attribute-validate-vlink",
      idxFrom: 13,
...
test(`08 - ${`\u001b[${36}m${"whitespace"}\u001b[${39}m`} - copious whitespace around - named`, () => {
  ...
  equal(applyFixes(str, messages), '<body vlink="PeachPuff">', "08.01");
  compare(ok, messages, [
    {
      ruleId: "attribute-validate-vlink",
      idxFrom: 13,
...
test(`11 - ${`\u001b[${35}m${"named"}\u001b[${39}m`} - healthy`, () => {
  let str = "<body class='zz' vlink='blue' id='yy aa'>"; // <-- notice single quotes
  let linter = new Linter();
  let messages = linter.verify(str, {
    rules: {
      "attribute-validate-vlink": 2,
...
test(`16 - ${`\u001b[${35}m${"rgba"}\u001b[${39}m`} - healthy`, () => {
  let str = '<body vlink="rgb(255, 0, 153)">';
  let linter = new Linter();
  let messages = linter.verify(str, {
    rules: {
      "attribute-validate-vlink": 2,
