...
function checkForWhitespace(str: string, idxOffset: number): Res {
  ...
  if (!str.length || !str[0].trim().length) {
    ...
    console.log(
      `022 checkForWhitespace(): ${`\u001b[${32}m${`SET`}\u001b[${39}m`} ${`\u001b[${33}m${`charStart`}\u001b[${39}m`} = ${JSON.stringify(
        charStart,
        null,
        4
      )}`
    ...
    if (!str.length || charStart === null) {
      ...
      charEnd = null;
      console.log(
        `032 checkForWhitespace(): PUSH "Missing value." error on [${+idxOffset}, ${
      ...
      console.log(
        `044 checkForWhitespace(): PUSH [${idxOffset}, ${
          idxOffset + charStart
        }]`
      );
  ...
  if (charEnd && !str[str.length - 1].trim()) {
    ...
    console.log(
      `055 checkForWhitespace(): SET ${`\u001b[${33}m${`charEnd`}\u001b[${39}m`} = ${JSON.stringify(
        charEnd,
        null,
        4
      )}`
  ...
  console.log(
    `064 checkForWhitespace(): FIY, ${`\u001b[${33}m${`gatheredRanges`}\u001b[${39}m`} = ${JSON.stringify(
      gatheredRanges,
      null,
      4
    )}`
  ...
  console.log(`084 checkForWhitespace(): END`);
