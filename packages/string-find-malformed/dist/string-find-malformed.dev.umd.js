...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.stringFindMalformed = {}));
...
function findMalformed(str, refStr, cb, originalOpts) {
    ...
    if (typeof opts.stringOffset === "string" &&
        /^\d*$/.test(opts.stringOffset)) {
        opts.stringOffset = Number(opts.stringOffset);
    }
    else if (!Number.isInteger(opts.stringOffset) ||
        opts.stringOffset < 0) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        for (let z = 0, len3 = pendingMatchesArr.length; z < len3; z++) {
            ...
            if (Array.isArray(pendingMatchesArr[z].pendingToCheck) &&
            ...
            else if (Array.isArray(pendingMatchesArr[z].pendingToCheck) &&
                pendingMatchesArr[z].pendingToCheck.length &&
