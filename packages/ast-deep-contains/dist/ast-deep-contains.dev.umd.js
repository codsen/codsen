...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.astDeepContains = factory());
  ...
  function deepContains(tree1, tree2, cb, errCb, originalOpts) {
    ...
    if (typeDetect(tree1) !== typeDetect(tree2)) {
      ...
      astMonkeyTraverse(tree2, function (key, val, innerObj, stop) {
        var current = val !== undefined ? val : key;
        var path = innerObj.path; // retrieve the path of the current node from the monkey
        // console.log(
        ...
        if (objectPath.has(tree1, path)) {
          if (!opts.arrayStrictComparison && isObj$1(current) && innerObj.parentType === "array" && innerObj.parent.length > 1) {
            ...
            if (!opts.skipContainers || !isObj$1(retrieved) && !isArr(retrieved)) {
