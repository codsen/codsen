...
rootPack.dependencies.lerna = "^2.10.0";
...
test.serial("01 - monorepo", async t => {
  ...
  await pMap(test1FilePaths, (oneOfTestFilePaths, testIndex) =>
    ...
    .then(contents => {
      ...
      // root package.json:
      // at the time of writing this, latest Lerna is 3.10.7. We set original
      // version in root as ^2.10.0, so check is, is the second digit greater than 2
      t.true(Number.parseInt(contents[3].dependencies.lerna.slice(1, 2)) > 2);
    })
...
test.serial("02 - normal repo", async t => {
  ...
  await pMap(test2FilePaths, (oneOfTestFilePaths, testIndex) =>
    ...
    .then(contents => {
      ...
      // root package.json:
      t.regex(contents[1].dependencies.lerna, /\^\d+\.\d+\.\d+/);
      t.regex(contents[1].devDependencies.husky, /\^\d+\.\d+\.\d+/);
      t.regex(contents[1].devDependencies.lerna, /\^\d+\.\d+\.\d+/);
      t.regex(contents[1].devDependencies.prettier, /\^\d+\.\d+\.\d+/);
    })
...
test.serial("03 - version output mode", async t => {
...
test.serial("04 - help output mode", async t => {
...
test.serial("05 - no files found in the given directory", async t => {
  const tempFolder = "temp";
  // create folder:
