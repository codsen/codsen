...
tap.test("01 - umd is indeed ES5", (t) => {
  ...
  const umdSource = fs.readFileSync(path.resolve("dist/string-strip-html.umd.js"), "utf8");
...
tap.test("02 - dev.umd is indeed ES5", (t) => {
  const linter = new Linter();
  const devUmdSource = fs.readFileSync(path.resolve("dist/string-strip-html.dev.umd.js"), "utf8");
  const messages = linter.verify(devUmdSource, {}, { allowInlineConfig: false });
...
tap.test("03 - cjs is indeed ES5", (t) => {
  const linter = new Linter();
  const cjsSource = fs.readFileSync(path.resolve("dist/string-strip-html.cjs.js"), "utf8");
