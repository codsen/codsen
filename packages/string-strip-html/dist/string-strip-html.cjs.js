...
function stripHtml(str, originalOpts) {
  ...
  for (var i = 0, len = str.length; i < len; i++) {
    ...
    if (tag.lastOpeningBracketAt !== undefined) {
      if (tag.lastClosingBracketAt === undefined) {
        if (str[i + 1] === undefined || str[i + 1] === "<") {
          ...
          rangesToDelete.push(tag.leftOuterWhitespace, i + 1, str.slice(tag.leftOuterWhitespace, i + 1).includes("\n") ? "\n" : " ");
        }
        ...
        // part 1.
        rangesToDelete.push(tag.leftOuterWhitespace, tag.lastClosingBracketAt === i ? i + 1 : i, str.slice(tag.leftOuterWhitespace, i).includes("\n") ? "\n" : " ");
        // part 2.
        if (str[i] !== ">") {
          tag = {};
        }
    ...
    // catch slash
    // -------------------------------------------------------------------------
    if (str[i] === "/" && tag.lastOpeningBracketAt !== undefined && tag.lastClosingBracketAt === undefined) {
