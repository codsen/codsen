...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.easyReplace = {}));
...
function iterateLeft(elem, arrSource, foundBeginningIndex, i) {
    ...
    for (let i2 = 0, len = charsArray.length; i2 < len; i2++) {
        ...
        if (i) {
            if (charsArray[i2].toLowerCase() !==
                arrSource[foundBeginningIndex - Array.from(elem).length + i2].toLowerCase()) {
                matched = false;
                break;
            }
        }
        else if (charsArray[i2] !==
            ...
            for (let i = 0, len = opts.rightMaybe.length; i < len; i++) {
                ...
                for (let i2 = 0, len2 = splitRightMaybe.length; i2 < len2; i2++) {
                    ...
                    if (opts.i.rightMaybe) {
                        if (splitRightMaybe[i2].toLowerCase() !==
                            arrSource[oneOfFoundIndexes + Array.from(opts.searchFor).length + i2].toLowerCase()) {
                            matched = false;
                            break;
                        }
                    }
                    else if (splitRightMaybe[i2] !==
                        arrSource[oneOfFoundIndexes + Array.from(opts.searchFor).length + i2]) {
                        matched = false;
                        break;
                    }
                }
