...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.astGetObject = {}));
...
function internalApi(originalAst, keyValPair, replacementContentsArr, result) {
  ...
  var ast = lodash_clonedeep(originalAst); // if object is passed, crawl it, checking for keyValPair:

  /* istanbul ignore else */

  if (lodash_isplainobject(ast)) {
    // console.log('\nwill compare:')
    ...
    if (compare(ast, keyValPair)) {
      ...
      Object.keys(ast).forEach(function (key) {
        if (Array.isArray(ast[key]) || lodash_isplainobject(ast[key])) {
    ...
    ast.forEach(function (_el, i) {
      ...
      /* istanbul ignore else */
      if (lodash_isplainobject(ast[i]) || Array.isArray(ast[i])) {
        if (set) {
