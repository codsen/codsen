...
const isArr = Array.isArray; // ECMA specification: http://www.ecma-international.org/ecma-262/6.0/#sec-tostring
...
function findUnusedSync(arrOriginal, originalOpts) {
  ...
  function findUnusedSyncInner(arr1, opts1, res = [], path = "") {
    ...
    if (arr1.every(el => isObj(el))) {
      ...
      if (arr1.length > 1) {
        const unusedKeys = Object.keys(keySet).filter(key => arr1.every(obj => (opts1 && obj[key] === opts1.placeholder || obj[key] === undefined) && (!opts1 || !opts1.comments || !includes(key, opts1.comments)))); // console.log(`unusedKeys = ${JSON.stringify(unusedKeys, null, 4)}`)

        res = res.concat(unusedKeys.map(el => `${path}.${el}`)); // console.log(`res = ${JSON.stringify(res, null, 4)}`)
      } // ------ PART 2 ------
      ...
      const keysContents = keys.map(key => typ(keySet[key])); // can't use map() because we want to prevent nulls being written.
