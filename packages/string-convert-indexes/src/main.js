...
function strConvertIndexes(mode, str, indexes, originalOpts) {
  ...
  if (!isStr(str) || (str.length === 0)) {
    throw new TypeError(`string-convert-indexes: [THROW_ID_02] the first input argument, input string, must be a non-zero-length string! Currently it's: ${typeof str}, equal to:\n${str}`)
  }
  if (existy(originalOpts) && !isObj(originalOpts)) {
    throw new TypeError(`string-convert-indexes: [THROW_ID_03] the third input argument, Optional Options Object, must be a plain object! Currently it's: ${typeof originalOpts}, equal to:\n${originalOpts}`)
  }
  // prep the opts
  const defaults = {
  ...
  for (let i = 0, len = str.length; i <= len; i++) {
    ...
    if (mode === 'n') {
      ...
      for (let y = 0, leny = toDoList.length; y < leny; y++) {
        ...
        if ((prep(toDoList[y].val) === unicodeIndex) && (toDoList[y].res === undefined)) {
  ...
  for (let z = toDoList.length; z--;) {
    ...
    res = set(res, { index: toDoList[z].id, val: toDoList[z].res !== undefined ? toDoList[z].res : toDoList[z].val })
  }
