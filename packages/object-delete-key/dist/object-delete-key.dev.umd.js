...
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.objectDeleteKey = {}));
...
function isArguments(value) {
  // Safari 8.1 makes `arguments.callee` enumerable in strict mode.
  return isArrayLikeObject$1(value) && hasOwnProperty$2.call(value, 'callee') && (!propertyIsEnumerable.call(value, 'callee') || objectToString$2.call(value) == argsTag);
}
/**
...
function isSymbol(value) {
  return typeof value == 'symbol' || isObjectLike$2(value) && objectToString$2.call(value) == symbolTag;
}
/**
...
function compare(b, s, originalOpts) {
  ...
  if (Array.isArray(b) && Array.isArray(s)) {
    ...
    var _loop = function _loop() {
      ...
      if (b[sKey] != null && typeDetect(b[sKey]) !== typeDetect(s[sKey])) {
        ...
        if (!(empty(b[sKey]) && empty(s[sKey]) && opts.hungryForWhitespace)) {
          ...
          return {
            v: "The given key " + sKey + " is of a different type on both objects. On the first-one, it's " + typeDetect(s[sKey]) + ", on the second-one, it's " + typeDetect(b[sKey])
          };
        }
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(sKeys), _step; !(_step = _iterator()).done;) {
      var _ret = _loop();
