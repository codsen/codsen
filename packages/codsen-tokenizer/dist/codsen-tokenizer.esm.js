...
function tokenizer(str, originalOpts) {
  ...
  for (let i = 0; i <= len; i++) { //
    ...
    property && property.valueStarts && !property.valueEnds) {
      ...
      !str[i] || // or it's erroneous whitespace:
      str[i] && !str[i].trim() || // normal head css styles:
    ...
    if (!doNothing && // token.type === "rule" &&
    property && property.colon && !property.valueStarts && str[i] && str[i].trim()) {
      /* istanbul ignore else */
      ...
      if (token.type === "tag" && !layers.length && str[i] === ">") {
        ...
        if (wholeEspTagClosing.length > token.head.length) { // in order for this to be tails + new heads, the total length should be
          ...
          if (wholeEspTagClosing.endsWith(token.head)) { // we have a situation like
            ...
            const secondPartOfWholeEspTagClosing = wholeEspTagClosing.slice(wholeEspTagClosing.indexOf(headsFirstChar)); // imagine we cliced off (Nunjucks): -%}{%-
    ...
    if (!doNothing && token.type === "tag" && attrib.attribValueStartsAt && i >= attrib.attribValueStartsAt && attrib.attribValueEndsAt === null) {
      ...
      if (SOMEQUOTE.includes(str[i])) { // const R1 = !layers.some((layerObj) => layerObj.type === "esp");
    ...
    if (str[i] === ">" && token.type === "tag" && attrib.attribStarts && !attrib.attribEnds) { // Idea is simple: we have to situations:
