...
tap.test(`30 - spaced chain, text-ESP, no important`, (t) => {
  const gathered = [];
...
tap.test(`31 - spaced chain, text-ESP, with important`, (t) => {
  const gathered = [];
...
tap.test(`37 - two tight esp tokens, with semi`, (t) => {
  const gathered = [];
...
tap.test(`38 - two tight esp tokens`, (t) => {
  const gathered = [];
  const input = '<div style="padding:{{ t }}\t{{ r }}">';
  ct(input, {
    tagCb: (obj) => {
...
tap.test(`40 - property without semi wrapped in ESP tokens`, (t) => {
  const gathered = [];
...
tap.test(`41 - property with semi wrapped in ESP tokens`, (t) => {
  const gathered = [];
  const input = `<td style="a: b;
    {% if x %}c: d;{% endif %}
e: f;">x</td>`;
...
tap.test(`42 - property + space wrapped in ESP tokens`, (t) => {
  const gathered = [];
  const input = `<td style="a: b;
    {% if x %}c: d {% endif %}
e: f;">x</td>`;
