  ...
  traverse(obj, function (key, val, innerObj) {
    ...
    if (opts.enforceStrictKeyset && isNotAnArrayChild && !optsSchemaHasThisPathDefined && !refHasThisPathDefined) {
      ...
      if (!lodash_intersection(currentKeysSchema, NAMESFORANYTYPE).length) {
        ...
        if (current !== true && current !== false && !currentKeysSchema.includes(typeDetect(current).toLowerCase()) || (current === true || current === false) && !currentKeysSchema.includes(String(current)) && !currentKeysSchema.includes("boolean")) { // new in v.2.2
