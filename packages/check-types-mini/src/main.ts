...
import { isPlainObject as isObj, pullAll } from "codsen-utils";
...
function internalApi(obj: Obj, ref: Obj | null, opts?: Partial<Opts>): void {
  ...
  if (isObj(resolvedOpts.schema)) {
    ...
    Object.keys(resolvedOpts.schema).forEach((oneKey) => {
      if (isObj(resolvedOpts.schema[oneKey])) {
        ...
        DEV &&
          console.log(
            `149 FINAL ${`\u001b[${33}m${`tempObj`}\u001b[${39}m`} = ${JSON.stringify(
              tempObj,
              null,
              4
            )}`
        ...
        DEV &&
          console.log(
            `164 FINALLY, ${`\u001b[${33}m${`resolvedOpts.schema`}\u001b[${39}m`} = ${JSON.stringify(
              resolvedOpts.schema,
              null,
              4
            )}`
  ...
  if (resolvedOpts.enforceStrictKeyset) {
    DEV &&
      console.log(
        `225 so \u001b[${31}m${`resolvedOpts.enforceStrictKeyset is ON`}\u001b[${39}m`
      );
    if (
  ...
  traverse(obj, (key, val, innerObj) => {
    ...
    DEV &&
      console.log(
        `365 ${`\u001b[${33}m${`resolvedOpts.schema`}\u001b[${39}m`} = ${JSON.stringify(
          resolvedOpts.schema,
          null,
          4
        )}`
...
current = ${JSON.stringify(current, null, 4)}\n\n`
      ...
      DEV &&
        console.log(
          `590 ${`\u001b[${33}m${`objectPath.get(resolvedOpts.schema, innerObj.path)`}\u001b[${39}m`} = ${JSON.stringify(
