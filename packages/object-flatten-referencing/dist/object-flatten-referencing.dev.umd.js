...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.objectFlattenReferencing = factory());
  ...
  */
  var isStringIntCjs = function isStringInt(str, option) {
    ...
    if (typeof str !== 'string') {
      throw new TypeError(str + ' is not a string. Argument must be a string to be checked if it represents an integer.');
    }
  ...
  function flattenObject(objOrig, opts) {
    ...
    var obj = lodash_clonedeep(objOrig);
    var res = [];
  ...
  function flattenArr(arrOrig, opts, wrap, joinArraysUsingBrs) {
    ...
    var arr = lodash_clonedeep(arrOrig);
    var res = "";
  ...
  function reclaimIntegerString(something) {
    if (isStr$1(something) && isStringIntCjs(something.trim())) {
  ...
  function outer(originalInput1, originalReference1, opts1) {
    ...
    if (existy$1(opts1) && !lodash_isplainobject(opts1)) {
    ...
    function ofr(originalInput, originalReference, originalOpts) {
      ...
      var defaults = {
        ...
        dontWrapKeys: [],
        dontWrapPaths: [],
        // More precise version of simple "dontWrapKeys" above. You can target
        ...
        xhtml: true,
        // when flattening arrays, put <br /> (XHTML) or <br> (HTML)
        preventDoubleWrapping: true,
        preventWrappingIfContains: [],
        objectKeyAndValueJoinChar: ".",
        wrapGlobalFlipSwitch: true,
        // Allow disabling the wrapping feature. Used on deeper branches.
        ignore: [],
        // Ignore these keys, don't flatten their values.
        whatToDoWhenReferenceIsMissing: 0,
        // 0 = leave that key's value as it is,
        // 1 = throw, 2 = flatten to string & wrap if wrapping feature is enabled
        mergeArraysWithLineBreaks: true,
        // when merging arrays, should we
        // add <br /> between the rows?
        mergeWithoutTrailingBrIfLineContainsBr: true,
        // if line already contains BR,
      ...
      if (lodash_isplainobject(input)) {
        Object.keys(input).forEach(function (key) {
          var currentPath = currentRoot + (currentRoot.length === 0 ? key : ".".concat(key)); // console.log(`* currentPath = ${JSON.stringify(currentPath, null, 4)}\n\n`)
          ...
          if (opts.ignore.length === 0 || !opts.ignore.includes(key)) {
            ...
            if (existy$1(reference[key]) || !existy$1(reference[key]) && opts.whatToDoWhenReferenceIsMissing === 2) {
              if (isArr$1(input[key])) {
                if (opts.whatToDoWhenReferenceIsMissing === 2 || isStr$2(reference[key])) {
                  // reference is string
                  ...
                  if (input[key].every(function (el) {
                    ...
                    input[key].forEach(function (oneOfElements) {
                      ...
                      if (Array.isArray(oneOfElements) && !oneOfElements.every(isStr$2)) {
                        allOK = false;
                      }
                    });
        ...
        if (isArr$1(reference)) {
          input.forEach(function (el, i) {
            if (existy$1(input[i]) && existy$1(reference[i])) {
