...
async function writePackageJson(receivedPackageJsonObj) {
  ...
  objectPath.set(receivedPackageJsonObj, "repository", mainRepoPath);
  if (
    receivedPackageJsonObj.homepage.includes("github") ||
    (receivedPackageJsonObj.homepage.includes("bitbucket") &&
      !receivedPackageJsonObj.homepage.includes("codsen/codsen"))
  ) {
    objectPath.set(receivedPackageJsonObj, "homepage", mainRepoPath);
  }
...
async function step10() {
  ...
  if (isObj(newValues) && Object.keys(newValues).length > 0) {
    ...
    if (objectPath.has(pack, "devDependencies.rollup")) {
      objectPath.set(finalThing, "main", `dist/${parsedPack.project}.cjs.js`);
      objectPath.set(finalThing, "module", `dist/${parsedPack.project}.esm.js`);
      objectPath.set(
        finalThing,
        "browser",
        `dist/${parsedPack.project}.umd.js`
      );
    ...
    if (
      ...
      for (let i = 0, len = adhocDevDeps.length; i < len; i++) {
        ...
        if (!objectPath.has(finalThing, `devDependencies.${dep}`)) {
          ...
          // console.log(
          //   `1121 lect/cli.js: ${`\u001b[${33}m${`retrievedVersion`}\u001b[${39}m`} = ${JSON.stringify(
          //     retrievedVersion,
          //     null,
          //     4
          //   )}`
