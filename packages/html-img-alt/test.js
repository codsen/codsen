...
test('00.02 - whitespace between XHTML closing slash and closing bracket', (t) => {
  t.deepEqual(
    alt('zzz<img        alt="123"    /  >yyy'), 'zzz<img alt="123" />yyy',
    '00.02.01 - one HTML tag, only excessive whitespace',
  )
  t.deepEqual(
    alt('z/zz<img        alt="/123/"    /  >y/yy'), 'z/zz<img alt="/123/" />y/yy',
...
test('04.02 - alt with only one double quote, three HTML tags', (t) => {
  ...
  t.deepEqual(
    alt('<img alt="legit quote: \'" ><img alt="legit quote: \'" ><img alt="legit quote: \'" >'),
...
test('08.04 - adds an ALT within a nunjucks-sprinkled HTML', (t) => {
  t.deepEqual(
    alt('<img {% if m.n_o %}class="x-y"{% else %}id="a db-c d" style="display: block;"{% endif %}></td>'),
