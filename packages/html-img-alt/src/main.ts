...
function alts(str: string, opts?: Partial<Opts>): string {
  ...
  if (typeof str !== "string") {
    throw new TypeError(
      `html-img-alt/alts(): [THROW_ID_01] Input must be string! Currently its type is: ${typeof str}, equal to: ${formatDiagnosticValue(str, 4)}`,
    );
  }
  if (opts && !isObj(opts)) {
    throw new TypeError(
      `html-img-alt/alts(): [THROW_ID_02] Options object must be a plain object! Currently its type is: ${typeof opts}, equal to: ${formatDiagnosticValue(opts, 4)}`,
    );
  }
