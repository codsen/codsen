  ...
  for (let i = 0, len = str.length; i < len; i++) {
    ...
    if (str[i] === "=") {
      ...
      if (
        ...
        console.log("155 wiping plausibleWithinQuotesRanges");
    ...
    if (whitespaceStarted && str[i].trim() !== "") {
      console.log("174 whitespace ends");
      // put up excessive whitespace for deletion

      console.log(
        `178 withinQuotes = ${JSON.stringify(withinQuotes, null, 4)}`
      );
      if (
        whitespaceStarted <
        i -
        ...
        if (!withinQuotes) {
          console.log(
            `195 add no.1.1 - adding whitespace to rangesArr: [${whitespaceStarted}, ${i -
              1 +
              (str[i] === ">" ||
              str[i] === "'" ||
              slashStartedAt ||
          ...
          console.log(
            `222 add no.1.2 - adding whitespace to plausibleWithinQuotesRanges: [${whitespaceStarted}, ${i -
              1 +
              (str[i] === ">" ||
              str[i] === "'" ||
              slashStartedAt ||
        ...
        if (str[i] === ">" || str[i] === "/") {
          ...
          console.log("251 SETTING finalSpaceNeeded = true");
        ...
        if (
          ...
          console.log(`272 add no.2 - adding ="" at location:${location}`);
          if (!withinQuotes) {
            rangesArr.add(
              location,
              location,
    ...
    if (str[i] === '"') {
      if (
        ...
        console.log("313 TRUE for: i >= thereShouldBeTheFirstDoubleQuoteHere");
        ...
        if (str[i + 1].trim() === "/" || str[i + 1].trim() === ">") {
          console.log(`326 Adding empty space on i + 1 = ${i + 1}`);
          addSpaceInTheFutureBeforeSlashOrBracket = true;
          console.log("328 SETTING finalSpaceNeeded = false");
        ...
        console.log("335 TRUE for: i >= thereShouldBeTheSecondDoubleQuoteHere");
        ...
        console.log("342 wiping plausibleWithinQuotesRanges");
        plausibleWithinQuotesRanges.wipe();
        ...
        if (str[i + 1] === ">" || str[i + 1] === "/") {
          addSpaceInTheFutureBeforeSlashOrBracket = true;
          console.log("350 SETTING finalSpaceNeeded = false");
          finalSpaceNeeded = false;
        }

        // reset altContentsStart
        console.log(
          `356 ALT TAG CONTENTS: >>>${str.slice(altContentsStart, i)}<<< (${
    ...
    if (withinImageTag && !withinQuotes && str[i] === "'") {
      console.log("374 putting up a rogue single quote for deletion");
      rangesArr.add(i, i + 1);
      if (str[i + 1] === "/" || str[i + 1] === ">") {
        addSpaceInTheFutureBeforeSlashOrBracket = true;
      }
    ...
    if (withinImageTag && str[i] === ">") {
      console.log(
        `385 complete img tag: ${str.slice(imageTagStartedAt, i + 1)}`
      );
      console.log(
        `388 thereShouldBeTheFirstDoubleQuoteHere = ${JSON.stringify(
          thereShouldBeTheFirstDoubleQuoteHere,
          null,
          4
        )}`
      ...
      console.log(
        `395 thereShouldBeTheSecondDoubleQuoteHere = ${JSON.stringify(
          thereShouldBeTheSecondDoubleQuoteHere,
          null,
          4
        )}`
      ...
      if (altBegins === null) {
        console.log("407 NO ALT ATTR!");
        console.log(
          `409 slashStartedAt = ${JSON.stringify(slashStartedAt, null, 4)}`
        );
        if (slashStartedAt) {
          // XHTML.
          console.log(
            `414 add no.3 - adding >>>alt="" <<< at slashStartedAt=${slashStartedAt}`
          );
          rangesArr.add(slashStartedAt, slashStartedAt, ' alt="" ');
        } else {
          // HTML.
          console.log(`419 add no.4 - adding >>> alt="" <<< at i=${i}`);
          rangesArr.add(i, i, ' alt="" ');
        }
        console.log("422 SETTING finalSpaceNeeded = false");
        finalSpaceNeeded = false;
      ...
      if (!slashStartedAt && thereShouldBeEqualCharacterHere === i) {
        ...
        // HTML
        console.log(`430 add no.5 - adding >>>="" <<< at i=${i}`);
        rangesArr.add(i, i, '="" ');
        console.log("432 SETTING finalSpaceNeeded = false");
        finalSpaceNeeded = false;
      } else if (slashStartedAt && thereShouldBeEqualCharacterHere === i - 1) {
        // if ALT has no equal and is right before closing bracket
        // XHTML
        console.log(`437 add no.6 - adding >>>="" <<< at i-1=${i}` - 1);
        rangesArr.add(i - 1, i - 1, '="" ');
        console.log("439 SETTING finalSpaceNeeded = false");
        finalSpaceNeeded = false;
      }
      ...
      if (
        ...
        // HTML
        console.log(`449 add no.7 - adding >>>="" <<< at i=${i}`);
        rangesArr.add(i, i, '"" ');
        addSpaceInTheFutureBeforeSlashOrBracket = false;
      } else if (
        slashStartedAt &&
        ...
        // XHTML
        console.log(`458 add no.8 - adding >>>="" <<< at i-1=${i}` - 1);
        rangesArr.add(i - 1, i - 1, '"" ');
        addSpaceInTheFutureBeforeSlashOrBracket = false;
      } else if (
        !slashStartedAt &&
        ...
        console.log(
          "467 add no.9 - adding the missing double quote before HTML closing bracket"
        );
        // HTML
        rangesArr.add(i, i, '"');
        console.log(
          `472 rangesArr.current() = ${JSON.stringify(
            rangesArr.current(),
            null,
            4
          )}`
        ...
        console.log("480 merging in plausibleWithinQuotesRanges");
        ...
        console.log(
          "501 add no.10 - adding the missing double quote before XHTML closing slash"
        );
        // XHTML
        ...
        console.log(
          `511 UPDATED rangesArr.current(): ${JSON.stringify(
            rangesArr.current(),
            null,
            4
          )}`
      ...
      if (finalSpaceNeeded || addSpaceInTheFutureBeforeSlashOrBracket) {
        if (slashStartedAt) {
          console.log(
            "535 add no.12.1 - FINAL - adding missing space at slashStartedAt (XHTML)"
          );
          rangesArr.add(slashStartedAt, slashStartedAt, " ");
        } else {
          console.log(
            "540 add no.12.2 - FINAL - adding missing space at i (HTML)"
          );
          rangesArr.add(i, i, " ");
        }
        console.log(
          `545 new rangesArr: ${JSON.stringify(rangesArr.current(), null, 4)}`
        );
      }
...
${`\u001b[${90}m${`plausibleWithinQuotesRanges.current() = ${plausibleWithinQuotesRanges.current()}`}\u001b[${39}m`}`);
  ...
  console.log("\n\n\n=============\n\n");
  console.log(
    `604 FINAL rangesArr.current() = ${JSON.stringify(
      rangesArr.current(),
      null,
      4
    )}\n\n\n\n\n\n`
