...
function alts(str, opts) {
  ...
  for (var i = 0, len = str.length; i < len; i++) {
    ...
    if (withinImageTag && str[i] === '/') {
      if (DEBUG) {
        console.log('!!! setting slashStartedAt = ' + i);
      }
      slashStartedAt = i;
    }
    ...
    if (str[i] === '=') {
      ...
      if (plausibleWithinQuotesRanges.current() && plausibleWithinQuotesRanges.current().length) {
        if (DEBUG) {
          console.log('!!! wiping plausibleWithinQuotesRanges');
        }
    ...
    if (whitespaceStarted && str[i].trim() !== '') {
      ...
      if (whitespaceStarted < i - 1 + (str[i] === '>' || str[i] === '\'' || slashStartedAt || thereShouldBeEqualCharacterHere || thereShouldBeTheFirstDoubleQuoteHere || thereShouldBeTheSecondDoubleQuoteHere ? 1 : 0)) {
        if (!withinQuotes) {
          if (DEBUG) {
            console.log('!!! add no.1.1 - adding whitespace to rangesArr: [' + whitespaceStarted + ', ' + (i - 1 + (str[i] === '>' || str[i] === '\'' || slashStartedAt || thereShouldBeEqualCharacterHere || thereShouldBeTheFirstDoubleQuoteHere || thereShouldBeTheSecondDoubleQuoteHere ? 1 : 0)) + ']');
          }
          ...
          if (DEBUG) {
            console.log('!!! add no.1.2 - adding whitespace to plausibleWithinQuotesRanges: [' + whitespaceStarted + ', ' + (i - 1 + (str[i] === '>' || str[i] === '\'' || slashStartedAt || thereShouldBeEqualCharacterHere || thereShouldBeTheFirstDoubleQuoteHere || thereShouldBeTheSecondDoubleQuoteHere ? 1 : 0)) + ']');
          }
        ...
        if (str[i] === '>' || str[i] === '/') {
          ...
          if (DEBUG) {
            console.log('!!! 92 SETTING finalSpaceNeeded = true');
          }
        ...
        if (thereShouldBeEqualCharacterHere && str[i] !== '=' && i >= thereShouldBeEqualCharacterHere || thereShouldBeTheFirstDoubleQuoteHere && str[i] !== '"' && i >= thereShouldBeTheFirstDoubleQuoteHere) {
          ...
          if (DEBUG) {
            console.log('!!! add no.2 - adding ="" at location:' + location);
          }
          if (!withinQuotes) {
    ...
    if (str[i] === '"') {
      if (thereShouldBeTheFirstDoubleQuoteHere && i >= thereShouldBeTheFirstDoubleQuoteHere) {
        ...
        if (str[i + 1].trim() === '/' || str[i + 1].trim() === '>') {
          if (DEBUG) {
            console.log('!!! Adding empty space on i + 1 = ' + (i + 1));
          }
          addSpaceInTheFutureBeforeSlashOrBracket = true;
          if (DEBUG) {
            console.log('!!! 93 SETTING finalSpaceNeeded = false');
          }
        ...
        if (str[i + 1] === '>' || str[i + 1] === '/') {
          addSpaceInTheFutureBeforeSlashOrBracket = true;
          if (DEBUG) {
            console.log('!!! 94 SETTING finalSpaceNeeded = false');
          }
          finalSpaceNeeded = false;
        }
    ...
    if (withinImageTag && !withinQuotes && str[i] === '\'') {
      if (DEBUG) {
        console.log('!!! putting up a rogue single quote for deletion');
      }
      rangesArr.add(i, i + 1);
      if (str[i + 1] === '/' || str[i + 1] === '>') {
        addSpaceInTheFutureBeforeSlashOrBracket = true;
    ...
    if (withinImageTag && str[i] === '>') {
      ...
      if (altBegins === null) {
        ...
        if (slashStartedAt) {
          // XHTML.
          if (DEBUG) {
            console.log('!!! add no.3 - adding >>>alt="" <<< at slashStartedAt=' + slashStartedAt);
          }
          rangesArr.add(slashStartedAt, slashStartedAt, ' alt="" ');
        } else {
          // HTML.
          if (DEBUG) {
            console.log('!!! add no.4 - adding >>> alt="" <<< at i=' + i);
          }
          rangesArr.add(i, i, ' alt="" ');
        }
        if (DEBUG) {
          console.log('!!! 95 SETTING finalSpaceNeeded = false');
        }
        finalSpaceNeeded = false;
      ...
      if (!slashStartedAt && thereShouldBeEqualCharacterHere === i) {
        ...
        // HTML
        if (DEBUG) {
          console.log('!!! add no.5 - adding >>>="" <<< at i=' + i);
        }
        rangesArr.add(i, i, '="" ');
        if (DEBUG) {
          console.log('!!! 96 SETTING finalSpaceNeeded = false');
        }
        finalSpaceNeeded = false;
        ...
        // XHTML
        if (DEBUG) {
          console.log('!!! add no.6 - adding >>>="" <<< at i-1=' + i - 1);
        }
        rangesArr.add(i - 1, i - 1, '="" ');
        if (DEBUG) {
          console.log('!!! 97 SETTING finalSpaceNeeded = false');
        }
        finalSpaceNeeded = false;
      }
      ...
      if (!slashStartedAt && thereShouldBeTheFirstDoubleQuoteHere && thereShouldBeTheFirstDoubleQuoteHere <= i) {
        // HTML
        if (DEBUG) {
          console.log('!!! add no.7 - adding >>>="" <<< at i=' + i);
        }
        rangesArr.add(i, i, '"" ');
        addSpaceInTheFutureBeforeSlashOrBracket = false;
        ...
        // XHTML
        if (DEBUG) {
          console.log('!!! add no.8 - adding >>>="" <<< at i-1=' + i - 1);
        }
        rangesArr.add(i - 1, i - 1, '"" ');
        addSpaceInTheFutureBeforeSlashOrBracket = false;
      } else if (!slashStartedAt && thereShouldBeTheSecondDoubleQuoteHere && thereShouldBeTheSecondDoubleQuoteHere <= i) {
        if (DEBUG) {
          console.log('!!! add no.9 - adding the missing double quote before HTML closing bracket');
        }
        // HTML
        rangesArr.add(i, i, '"');
        if (DEBUG) {
          console.log('* rangesArr.current() = ' + JSON.stringify(rangesArr.current(), null, 4));
        }
        addSpaceInTheFutureBeforeSlashOrBracket = true;
        // so if the second double quote is missing, merge in the plausible ranges, if any
        if (DEBUG) {
          console.log('!!! merging in plausibleWithinQuotesRanges');
        }
        ...
        if (DEBUG) {
          console.log('!!! add no.10 - adding the missing double quote before XHTML closing slash');
        }
        // XHTML
      ...
      if (finalSpaceNeeded || addSpaceInTheFutureBeforeSlashOrBracket) {
        if (slashStartedAt) {
          if (DEBUG) {
            console.log('!!! add no.12.1 - FINAL - adding missing space at slashStartedAt (XHTML)');
          }
          rangesArr.add(slashStartedAt, slashStartedAt, ' ');
        } else {
          if (DEBUG) {
            console.log('!!! add no.12.2 - FINAL - adding missing space at i (HTML)');
          }
          rangesArr.add(i, i, ' ');
        }
        if (DEBUG) {
          console.log('new rangesArr: ' + JSON.stringify(rangesArr.current(), null, 4));
        }
      }
    ...
    if (whitespaceStarted) {
      if (DEBUG) {
        console.log('!!! whitespaceStarted = ' + JSON.stringify(whitespaceStarted, null, 4));
      }
    }
    if (DEBUG) {
