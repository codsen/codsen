...
function rIterate(str, originalRanges, cb, offset = 0) {
  ...
  if (originalRanges === null || !originalRanges.length) {
    ...
    const ranges = Array.from(originalRanges); // currentIdx is index we use to traverse the string. It's like "i" above,
    // except it "pauses" and is fired many times when range has third argument,
    // what to insert.

    let currentIdx = offset; // currentIdx is the index of the final string with all ranges applied (
    ...
    let finalIdx = offset; // cover the first characters up to starting range

    if (finalIdx < ranges[0][0]) { // eslint-disable-next-line
    ...
    if (ranges[0][0] <= currentIdx) {
      ranges.forEach((rangeArr, rangeArrIdx) => { // 1. if "to insert" value, third range's argument is given, loop through it,
