...
function rangesIterate(str, ranges, cb) {
  ...
  if (ranges !== null && !isArr(ranges)) {
    throw new TypeError("ranges-iterate: [THROW_ID_03] Input ranges must be an array, consisting of zero or more arrays! Currently its type is: ".concat(_typeof(ranges), ", equal to: ").concat(JSON.stringify(ranges, null, 0)));
  }
  if (!cb) {
