...
for (const which of [
  ...
  test(`validates babel config: ${which}`, t => {
    execCli(
      ["es2015.js"],
      { dirname: `fixture/invalid-babel-config/${which}` },
      (err, stdout, stderr) => {
        t.ok(err);
...
test("includes relative paths in source map", t => {
  execCli([], { dirname: "fixture/correct-sources-in-source-map" }, () => {
    const [file] = globby.sync(["*.js.map"], {
      ...
      cwd: path.resolve(
        __dirname,
        "../fixture/correct-sources-in-source-map/node_modules/.cache/ava"
      )
    });
    ...
    t.is(
      map.sourceRoot,
      path.resolve(__dirname, "../fixture/correct-sources-in-source-map")
    );
    t.end();
  });
});
...
for (const plugin of [
  ...
  test(`avoids applying '@babel/plugin-syntax-${plugin}' if already in config`, t => {
    execCli(
      [],
      { dirname: `fixture/babel/with-explicit-syntax-plugins/${plugin}` },
      err => {
        t.ifError(err);
        t.end();
      }
