...
for (const obj of [
  ...
  test(`snapshots work (${obj.type})`, t => {
    ...
    execCli(
      ["--update-snapshots", "--verbose"],
      { dirname, env: overrideCIChecks },
      error => {
        t.ifError(error);
...
test("snapshots infer their location and name from sourcemaps", t => {
  ...
  execCli(
    ["--verbose"],
    { dirname: relativeFixtureDir, env: overrideCIChecks },
    (error, stdout) => {
      t.ifError(error);
...
test("snapshots are indentical on different platforms", t => {
  ...
  execCli(
    ["--update-snapshots", "--verbose"],
    { dirname: fixtureDir, env: overrideCIChecks },
    error => {
      t.ifError(error);
      ...
      t.true(
        reportContents.equals(expectedReportContents),
        "report file contents matches snapshot"
      );
      t.true(
        snapContents.equals(expectedSnapContents),
        "snap file contents matches snapshot"
      );
      t.end();
    }
  );
