...
test("watcher reruns test files when they changed", t => {
  ...
  const child = execCli(
    ...
    { dirname: "fixture/watcher", env: { CI: "" } },
    err => {
      t.ok(killed);
      t.ifError(err);
      t.end();
...
test("watcher respects custom test file extensions", t => {
  ...
  const child = execCli(
    ["--verbose", "--watch"],
    { dirname: "fixture/watcher/custom-extensions", env: { CI: "" } },
    err => {
      t.ok(killed);
      t.ifError(err);
      t.end();
  ...
  child.stdout.on("data", str => {
    ...
    if (buffer.includes("1 test passed")) {
      if (!passedFirst) {
        touch.sync(
          path.join(__dirname, "../fixture/watcher/custom-extensions/test.foo")
        );
        buffer = "";
        passedFirst = true;
      } else if (!killed) {
...
test("watcher reruns test files when source dependencies change", t => {
  ...
  const child = execCli(
    ["--verbose", "--watch", "test-1.js", "test-2.js"],
    { dirname: "fixture/watcher/with-dependencies", env: { CI: "" } },
    err => {
      t.ok(killed);
      t.ifError(err);
      t.end();
...
test("watcher reruns ONLY test files that depend on a changed source with custom extension", t => {
  ...
  const child = execCli(
    ...
    {
      dirname: "fixture/watcher/with-custom-ext-dependencies",
      env: { CI: "" }
    },
    err => {
      t.ok(killed);
  ...
  child.stdout.on("data", str => {
    ...
    if (buffer.includes("2 tests passed") && !passedFirst) {
      touch.sync(
        path.join(
          __dirname,
          "../fixture/watcher/with-custom-ext-dependencies/source.custom-ext"
        )
      );
      buffer = "";
      passedFirst = true;
  ...
  child.stdout.on("data", str => {
    ...
    if (buffer.includes("2 tests passed") && !passedFirst) {
      touch.sync(
        path.join(
          __dirname,
          "../fixture/watcher/with-custom-ext-dependencies/setup.js"
        )
      );
      buffer = "";
      passedFirst = true;
...
test("watcher does not rerun test files when they write snapshot files", t => {
  ...
  const child = execCli(
    ...
    { dirname: "fixture/snapshots", env: { CI: "" } },
    err => {
      t.ok(killed);
      t.ifError(err);
      t.end();
  ...
  child.stdout.on("data", str => {
    ...
    if (buffer.includes("2 tests passed") && !passedFirst) {
      ...
      t.is(
        buffer.replace(/\s/g, "").replace(END_MESSAGE.replace(/\s/g, ""), ""),
        ""
      );
    }
  });
...
test("watcher does not rerun test files when files change that are neither tests, helpers nor sources", t => {
  ...
  const child = execCli(
    ["--verbose", "--watch"],
    { dirname: "fixture/watcher/ignored-files", env: { CI: "" } },
    err => {
      t.ok(killed);
      t.ifError(err);
      t.end();
  ...
  child.stdout.on("data", str => {
    ...
    if (buffer.includes("1 test passed") && !passedFirst) {
      touch.sync(
        path.join(__dirname, "../fixture/watcher/ignored-files/ignored.js")
      );
      buffer = "";
      passedFirst = true;
      setTimeout(() => {
...
test('`"tap": true` config is ignored when --watch is given', t => {
  ...
  const child = execCli(
    ...
    { dirname: "fixture/watcher/tap-in-conf", env: { CI: "" } },
    () => {
      t.ok(killed);
      t.end();
    }
