...
test('errors if top-level extensions include "js" without babel=false', t => {
  execCli(
    ["es2015.js"],
    { dirname: "fixture/invalid-extensions/top-level" },
    (err, stdout, stderr) => {
      t.ok(err);
      ...
      expectedOutput +=
        figures.cross +
        " Cannot specify generic 'js' extension without disabling AVA's Babel usage.";
      expectedOutput += "\n";
...
for (const [where, which, msg = "'js', 'jsx'"] of [
  ...
  test(`errors if ${where} extensions include duplicates`, t => {
    execCli(
      ["es2015.js"],
      { dirname: `fixture/invalid-extensions/${which}` },
      (err, stdout, stderr) => {
        t.ok(err);
        ...
        expectedOutput +=
          figures.cross +
          ` Unexpected duplicate extensions in options: ${msg}.`;
        expectedOutput += "\n";
