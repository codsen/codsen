...
test(
  "verbose reporter - typescript",
  run("typescript", [report.sanitizers.lineEndings])
);
...
test("verbose reporter - timeout", t => {
  ...
  t.test(
    "single file with only certain tests matched run",
    run("timeoutWithMatch")
  );
  t.end();
});
