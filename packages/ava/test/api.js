...
const testCapitalizerPlugin = require.resolve(
  "./fixture/babel-plugin-test-capitalizer"
);
...
test("ES2015 support", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/es2015.js")])
    .then(runStatus => {
...
test("async/await support", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/async-await.js")])
    .then(runStatus => {
...
test("test.meta.file", t => {
  ...
  return api.run([path.join(__dirname, "fixture/meta.js")]).then(runStatus => {
    t.is(runStatus.stats.passedTests, 2);
  });
});
...
test("fail-fast mode - single file & serial", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/fail-fast/single-file/test.js")])
    .then(runStatus => {
      ...
      t.strictDeepEqual(tests, [
        {
          ...
          testFile: path.join(
            __dirname,
            "fixture/fail-fast/multiple-files/fails.js"
          ),
          title: "first pass"
        },
        {
    ...
    .then(runStatus => {
      ...
      t.strictDeepEqual(tests, [
        ...
        },
        {
          ...
          testFile: path.join(
            __dirname,
            "fixture/fail-fast/multiple-files/passes-slow.js"
          ),
          title: "first pass"
        }
      ]);
...
test("serial execution mode", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/serial.js")])
    .then(runStatus => {
...
test("run from package.json folder by default", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/process-cwd-default.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("control worker's process.cwd() with projectDir option", t => {
  ...
  return api.run([fullPath]).then(runStatus => {
    t.is(runStatus.stats.passedTests, 1);
  });
});
...
test("stack traces for exceptions are corrected using a source map file", t => {
  ...
  api.on("run", plan => {
    plan.status.on("stateChange", evt => {
      if (evt.type === "uncaught-exception") {
        ...
        t.match(
          evt.err.stack,
          /^.*?Object\.t.*?as run\b.*source-map-fixtures.src.throws.js:1.*$/m
        );
  ...
  return api
    .run([path.join(__dirname, "fixture/source-map-file.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("stack traces for exceptions are corrected using a source map file in what looks like a browser env", t => {
  ...
  api.on("run", plan => {
    plan.status.on("stateChange", evt => {
      if (evt.type === "uncaught-exception") {
        ...
        t.match(
          evt.err.stack,
          /^.*?Immediate\b.*source-map-file-browser-env.js:7.*$/m
        );
      }
    });
  });
  ...
  return api
    .run([path.join(__dirname, "fixture/source-map-file-browser-env.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("enhanced assertion formatting necessary whitespace and empty strings", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/enhanced-assertion-formatting.js")])
    .then(runStatus => {
...
test("stack traces for exceptions are corrected using a source map, taking an initial source map for the test file into account (cache on)", t => {
  ...
  api.on("run", plan => {
    plan.status.on("stateChange", evt => {
      if (evt.type === "uncaught-exception") {
        ...
        t.match(
          evt.err.stack,
          /^.*?Immediate\b.*source-map-initial-input.js:14.*$/m
        );
      }
    });
  });
  ...
  return api
    .run([path.join(__dirname, "fixture/source-map-initial.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("absolute paths", t => {
  ...
  return api.run([path.resolve("test/fixture/es2015.js")]).then(runStatus => {
    t.is(runStatus.stats.passedTests, 1);
  });
});
...
test("symlink to test file directly", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/symlinkfile.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("test file in node_modules is ignored", t => {
  ...
  const api = apiCreator();
  return api
    .run([path.join(__dirname, "fixture/ignored-dirs/node_modules/test.js")])
    .then(runStatus => {
...
test("test file in helpers is ignored", t => {
  ...
  const api = apiCreator({
    helpers: ["**/helpers/*"],
    projectDir: path.join(__dirname, "fixture/ignored-dirs")
  });
  return api.run().then(runStatus => {
...
test("Node.js-style --require CLI argument", t => {
  const requirePath =
    "./" +
    path
      .relative(".", path.join(__dirname, "fixture/install-global.js"))
      .replace(/\\/g, "/");
  ...
  return api
    .run([path.join(__dirname, "fixture/validate-installed-global.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
...
test("caching is enabled by default", t => {
  ...
  return api.run([path.join(__dirname, "fixture/caching/test.js")]).then(() => {
    const files = fs.readdirSync(
      path.join(__dirname, "fixture/caching/node_modules/.cache/ava")
    );
...
test("test file with only skipped tests does not create a failure", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/skip-only.js")])
    .then(runStatus => {
...
test("test file with only skipped tests does not run hooks", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/hooks-skipped.js")])
    .then(runStatus => {
      t.is(runStatus.stats.selectedTests, 1);
      t.is(runStatus.stats.skippedTests, 1);
      t.is(runStatus.stats.passedTests, 0);
...
test("babel.testOptions with a custom plugin", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/babelrc/test.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    }, t.threw);
});
...
test('uses "development" Babel environment if NODE_ENV is the empty string', t => {
  ...
  return withNodeEnv("", () => api.run()).then(runStatus => {
    t.is(runStatus.stats.passedTests, 1);
  });
});
...
function generatePassDebugTests(execArgv) {
  test(`pass ${execArgv.join(" ")} to fork`, t => {
    ...
    return api._computeForkExecArgv().then(result => {
...
function generatePassInspectIntegrationTests(execArgv) {
  test(`pass ${execArgv.join(" ")} to fork`, t => {
    const api = apiCreator({ testOnlyExecArgv: execArgv });
    return api
      .run([path.join(__dirname, "fixture/inspect-arg.js")])
      .then(runStatus => {
        t.is(runStatus.stats.passedTests, 1);
      });
  });
...
test("`esm` package support", t => {
  ...
  return api
    .run([path.join(__dirname, "fixture/esm-pkg/test.js")])
    .then(runStatus => {
      t.is(runStatus.stats.passedTests, 1);
    });
});
