...
exports.sanitizers = {
  ...
  experimentalWarning: str =>
    str.replace(/^\(node:\d+\) ExperimentalWarning.+\n/g, ""),
  lineEndings: str => replaceString(str, "\r\n", "\n"),
  posix: str => replaceString(str, "\\", "/"),
  slow: str => str.replace(/(slow.+?)\(\d+m?s\)/g, "$1 (000ms)"),
  timeout: str =>
    replaceString(str, "Timeout._onTimeout", "Timeout.setTimeout"),
...
const run = (type, reporter, match = []) => {
  ...
  // Mimick watch mode
  return api
    .run(files, { clearLogOnNextRun: false, previousFailures: 0, runVector: 1 })
    .then(() => {
      reporter.endRun();
...
exports.timeoutInMultipleFiles = reporter =>
  run("timeoutInMultipleFiles", reporter);
exports.timeoutWithMatch = reporter =>
  run("timeoutWithMatch", reporter, ["*needle*"]);
