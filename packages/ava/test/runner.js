...
test("test types and titles", t => {
  ...
  const check = (setup, expect) => {
    ...
    runner.on("stateChange", evt => {
      if (
        evt.type === "hook-failed" ||
        evt.type === "test-failed" ||
        evt.type === "test-passed"
      ) {
  ...
  return Promise.all([
    ...
    check(
      ...
      [{ type: "test", title: "test" }, { type: "after", title: "after hook" }]
    ),
    check(
      chain => {
        chain("test", pass);
...
test("macros: Customize test names attaching a `title` function", t => {
  ...
  const expectedTitles = ["defaultA", "suppliedB", "defaultC"];
...
test("arrays of macros", t => {
  const expectedArgsA = [["A"], ["B"], ["C"]];

  const expectedArgsB = [["A"], ["B"], ["D"]];
