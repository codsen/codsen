...
const formatGlobals = Boolean(
  require(process.env.AVA_PATH + "/package.json").dependencies.concordance
);
...
test("date diff, extra properties", t => {
  t.deepEqual(
    ...
    Object.assign(new Date("1969-07-20T20:17:40.000Z"), {
      foo: "bar"
    })
  );
});
...
test("error formatted, constructor does not match name", t => {
  const err = Object.assign(new Error("Houston, we have a problem"), {
    name: "FamousWords"
  });
  t.true(err);
});
...
test("error diff, message", t => {
  t.deepEqual(
    ...
    new Error("One small step")
  );
});
test("error diff, constructor", t => {
  t.deepEqual(
    new Error("Houston, we have a problem"),
    new RangeError("One small step")
  );
});
test("error diff, extra properties", t => {
  t.deepEqual(
    new Error("Houston, we have a problem"),
    Object.assign(new Error("Houston, we have a problem"), {
...
test("map diff, extra properties", t => {
  t.deepEqual(
    new Map([["foo", "bar"]]),
    Object.assign(new Map([["foo", "bar"]]), { baz: "qux" })
  );
});
...
test("promise diff, extra properties", t => {
  t.deepEqual(
    Promise.resolve(),
    Object.assign(Promise.resolve(), { foo: "bar" })
  );
});
...
test("set diff, extra properties", t => {
  t.deepEqual(
    new Set([{ foo: "bar" }]),
    Object.assign(new Set([{ foo: "bar" }]), { baz: "qux" })
  );
});
...
test("buffer diff", t => {
  t.deepEqual(
    ...
    Buffer.from("baddecaf", "hex")
  );
});
test("buffer diff, extra properties", t => {
  t.deepEqual(
    Buffer.from("decafba".repeat(12), "hex"),
    Object.assign(Buffer.from("decafba".repeat(12), "hex"), { foo: "bar" })
  );
});
