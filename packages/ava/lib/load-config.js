...
function loadConfig({
  ...
  try {
    ...
    if (error && error.code === "MODULE_NOT_FOUND") {
      ...
      throw Object.assign(new Error(`Error loading ${fileForErrorMessage}`), {
        parent: error
      });
    }
  }
  ...
  if (fileConf === MISSING_DEFAULT_EXPORT) {
    throw new Error(
      `${fileForErrorMessage} must have a default export, using ES module syntax`
    );
  }
  ...
  if (fileConf !== NO_SUCH_FILE) {
    ...
    if (typeof fileConf === "function") {
      ...
      if (fileConf && typeof fileConf.then === "function") {
        // eslint-disable-line promise/prefer-await-to-then
        throw new TypeError(
          `Factory method exported by ${fileForErrorMessage} must not return a promise`
        );
      }
    ...
    if ("ava" in fileConf) {
      throw new Error(
        `Encountered 'ava' property in ${fileForErrorMessage}; avoid wrapping the configuration`
      );
    }
  }
