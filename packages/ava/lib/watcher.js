...
class Debouncer {
  ...
  debounce(delay) {
    ...
    delay = delay
      ? Math.max(delay, MIN_DEBOUNCE_DELAY)
...
class Watcher {
  constructor({ reporter, api, files, globs, resolveTestsFrom }) {
    ...
    this.run = (specificFiles = files, updateSnapshots) => {
      ...
      if (specificFiles.length > 0) {
        const exclusiveFiles = specificFiles.filter(file =>
          this.filesWithExclusiveTests.includes(file)
        );
        runOnlyExclusive =
          exclusiveFiles.length !== this.filesWithExclusiveTests.length;
        if (runOnlyExclusive) {
  ...
  trackTestDependencies(api) {
    ...
    api.on("run", plan => {
      plan.status.on("stateChange", evt => {
        ...
        const dependencies = evt.dependencies
          .map(x => relative(x))
          .filter(filePath => {
  ...
  updateTestDependencies(file, dependencies) {
    if (dependencies.length === 0) {
      this.testDependencies = this.testDependencies.filter(
        dep => dep.file !== file
      );
      return;
    }
  ...
  trackExclusivity(api) {
    api.on("run", plan => {
      plan.status.on("stateChange", evt => {
        ...
        this.updateExclusivity(
          evt.testFile,
          fileStats.declaredTests > fileStats.selectedTests
        );
      });
    });
  }
  ...
  pruneFailures(files) {
    ...
    this.filesWithFailures = this.filesWithFailures.filter(
      state => !toPrune.has(state.file)
    );
  }
  ...
  runAfterChanges() {
    ...
    for (const filePath of dirtyPaths) {
      const { isHelper, isSource, isTest } = globs.classify(
        filePath,
        this.globs
      );
    ...
    const addedOrChangedTests = dirtyTests.filter(
      path => dirtyStates[path] !== "unlink"
    );
    ...
    const testsByHelpersOrSource = dirtyHelpersAndSources
      .map(path => {
        return this.testDependencies
          .filter(dep => dep.contains(path))
          .map(dep => {
    ...
    if (testsByHelpersOrSource.length !== dirtyHelpersAndSources.length) {
      debug(
        "Helpers & sources remain that cannot be traced to specific tests: %O",
