...
function hasValidKeys(conf) {
  return Object.keys(conf).every(
    key => key === "extensions" || key === "testOptions"
  );
}
...
function validate(conf) {
  ...
  if (
    ...
    throw new Error(
      `Unexpected Babel configuration for AVA. See https://github.com/avajs/ava/blob/v${pkg.version}/docs/recipes/babel.md for allowed values.`
    );
  }
...
function hashPartialTestConfig(
  { babelrc, config, options: { plugins, presets } },
  projectDir,
  pluginAndPresetHashes
) {
  const inputs = [];
  if (babelrc) {
    inputs.push(babelrc);
    ...
    if (filename === "package.json") {
      inputs.push(
        JSON.stringify(pkgConf.sync("babel", { cwd: path.dirname(filename) }))
      );
    } else {
  ...
  for (const {
    ...
    if (firstComponent === "node_modules") {
      hash = packageHash.sync(
        findUp.sync("package.json", { cwd: path.dirname(filename) })
      );
    } else {
...
function build(projectDir, cacheDir, userOptions, compileEnhancements) {
  ...
  const containsAsyncGenerators = makeValueChecker(
    "@babel/plugin-syntax-async-generators"
  );
  const containsObjectRestSpread = makeValueChecker(
    "@babel/plugin-syntax-object-rest-spread"
  );
  const containsOptionalCatchBinding = makeValueChecker(
    "@babel/plugin-syntax-optional-catch-binding"
  );
  const containsStage4 = makeValueChecker("../stage-4");
  const containsTransformTestFiles = makeValueChecker(
  ...
  const loadOptions = filename => {
    ...
    if (!testOptions.plugins.some(containsAsyncGenerators)) {
      // TODO: Remove once Babel can parse this syntax unaided.
      testOptions.plugins.unshift(
        createConfigItem("@babel/plugin-syntax-async-generators", "plugin")
      );
    }

    if (!testOptions.plugins.some(containsObjectRestSpread)) {
      // TODO: Remove once Babel can parse this syntax unaided.
      testOptions.plugins.unshift(
        createConfigItem("@babel/plugin-syntax-object-rest-spread", "plugin")
      );
    }

    if (!testOptions.plugins.some(containsOptionalCatchBinding)) {
      // TODO: Remove once Babel can parse this syntax unaided.
      testOptions.plugins.unshift(
        createConfigItem(
          "@babel/plugin-syntax-optional-catch-binding",
    ...
    if (
      ...
      // Apply first.
      testOptions.presets.push(
        createConfigItem("@ava/babel-preset-transform-test-files", "preset", {
          powerAssert: true
        })
      );
    }

    const hash = hashPartialTestConfig(
      partialTestConfig,
      projectDir,
      pluginAndPresetHashes
    );
