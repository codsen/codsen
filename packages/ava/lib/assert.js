...
function formatDescriptorDiff(actualDescriptor, expectedDescriptor, options) {
  ...
  return {
    label: "Difference:",
    formatted: concordance.diffDescriptors(
      actualDescriptor,
      expectedDescriptor,
      options
    )
...
class Assertions {
  constructor({
    ...
    const withPowerAssert = (pattern, assertionFn) =>
      empower(assertionFn, {
        onError: event => {
          if (event.powerAssertContext) {
            event.error.statements = enhanceAssert.formatter(
    ...
    this.is = withSkip((actual, expected, message) => {
      ...
      if (Object.is(actual, expected)) {
        ...
        const result = concordance.compare(
          actual,
          expected,
          concordanceOptions
        );
    ...
    this.throwsAsync = withSkip((...args) => {
      ...
      const handlePromise = (promise, wasReturned) => {
        ...
        const intermediate = Promise.resolve(promise).then(
          value => {
            ...
            throw new AssertionError({
              ...
              values: [
                formatWithLabel(
                  `${
                    wasReturned ? "Returned promise" : "Promise"
                  } resolved with:`,
                  value
                )
              ]
      ...
      if (result.pass) {
        ...
        fail(
          new AssertionError({
            ...
            values: [
              formatDescriptorDiff(result.actual, result.expected, {
                invert: true
              })
            ]
          })
    ...
    this.assert = withSkip(
      withPowerAssert("assert(value, [message])", (actual, message) => {
