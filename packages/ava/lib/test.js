...
class Test {
  constructor(options) {
    ...
    this.compareWithSnapshot = assertionOptions => {
      ...
      const label = assertionOptions.id
        ? ""
    ...
    this.skipSnapshot = () => {
      if (options.updateSnapshots) {
        this.addFailedAssertion(
          new Error(
            "Snapshot assertions cannot be skipped when updating snapshots"
          )
        );
      } else {
  ...
  bindEndCallback() {
    ...
    throw new Error(
      '`t.end()`` is not supported in this context. To use `t.end()` as a callback, you must use "callback mode" via `test.cb(testName, fn)`'
    );
  }
  ...
  countPassedAssertion() {
    if (this.finishing) {
      this.saveFirstError(
        new Error("Assertion passed, but test has already finished")
      );
    }
  ...
  addFailedAssertion(error) {
    if (this.finishing) {
      this.saveFirstError(
        new Error("Assertion failed, but test has already finished")
      );
    }
  ...
  verifyPlan() {
    if (
      !this.assertError &&
      this.planCount !== null &&
      this.planCount !== this.assertCount
    ) {
      this.saveFirstError(
        new assert.AssertionError({
          assertion: "plan",
          message: `Planned for ${this.planCount} ${plur(
            "assertion",
            this.planCount
          )}, but got ${this.assertCount}.`,
          operator: "===",
  ...
  verifyAssertions() {
    if (!this.assertError) {
      if (
        this.failWithoutAssertions &&
        !this.calledEnd &&
        this.planCount === null &&
        this.assertCount === 0
      ) {
        this.saveFirstError(
          new Error("Test finished without running any assertions")
        );
      } else if (this.pendingAssertionCount > 0) {
        this.saveFirstError(
          new Error("Test finished, but an assertion is still pending")
        );
      }
    }
  }
  ...
  detectImproperThrows(error) {
    ...
    if (error) {
      values.push(
        formatErrorValue(
          `The following error was thrown, possibly before \`t.${pending.assertion}()\` could be called:`,
          error
        )
      );
    }
  ...
  run() {
    ...
    if (this.metadata.callback) {
      if (returnedObservable || returnedPromise) {
        ...
        this.saveFirstError(
          new Error(
            `Do not return ${asyncType} from tests declared via \`test.cb(...)\`, if you want to return a promise simply declare the test via \`test(...)\``
          )
        );
        return this.finishPromised();
      }
