...
class Api extends Emittery {
  constructor(options) {
    ...
    this._regexpFullExtensions = new RegExp(
      `\\.(${this.options.extensions.full
  ...
  async run(files = [], runtimeOptions = {}) {
    files = files.map(file =>
      path.resolve(this.options.resolveTestsFrom, file)
    );
    ...
    try {
      ...
      if (files.length === 0 || precompiler.enabled) {
        ...
        if (precompiler.enabled) {
          found = await globs.findHelpersAndTests({
            dev: apiOptions.dev,
      ...
      runStatus.emitStateChange({
        type: "internal-error",
        err: serializeError("Internal error", false, error)
      });
    }
  ...
  _setupPrecompiler() {
    ...
    const precompileFull = babelConfig
      ...
      : filename => {
          throw new Error(
            `Cannot apply full precompilation, possible bad usage: ${filename}`
          );
        };
    ...
    if (compileEnhancements) {
      precompileEnhancementsOnly =
        this.options.extensions.enhancementsOnly.length > 0
          ? babelPipeline.build(projectDir, cacheDir, null, compileEnhancements)
          : filename => {
              throw new Error(
                `Cannot apply enhancement-only precompilation, possible bad usage: ${filename}`
              );
            };
    }
  ...
  async _computeForkExecArgv() {
    ...
    const inspectArgIndex = execArgv.findIndex(arg =>
