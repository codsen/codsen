...
function normalizeGlobs(
  ...
  if (
    typeof testPatterns !== "undefined" &&
    (!Array.isArray(testPatterns) || testPatterns.length === 0)
  ) {
    throw new Error(
  ...
  if (
    typeof helperPatterns !== "undefined" &&
    (!Array.isArray(helperPatterns) || helperPatterns.length === 0)
  ) {
    throw new Error(
      "The 'helpers' configuration must be an array containing glob patterns."
    );
  }

  if (
    sourcePatterns &&
    (!Array.isArray(sourcePatterns) || sourcePatterns.length === 0)
  ) {
    throw new Error(
      "The 'sources' configuration must be an array containing glob patterns."
    );
  }
...
async function findHelpersAndTests({
  cwd,
  extensions,
  testPatterns,
  helperPatterns
  ...
  for (const file of await findingTests) {
    ...
    if (path.basename(file).startsWith("_")) {
      uniqueHelpers.add(file);
    } else if (!uniqueHelpers.has(file)) {
      // Helpers cannot be tests.
      tests.push(file);
    }
  }
...
async function findTests({ cwd, extensions, testPatterns, helperPatterns }) {
  ...
  for (const file of await findFiles(cwd, testPatterns)) {
    ...
    if (
      rejectHelpers &&
      matches(normalizeFileForMatching(cwd, file), helperPatterns)
    ) {
      continue;
    }
...
const normalizeFileForMatching = (cwd, file) => {
  ...
  if (!cwd) {
    // TODO: Ensure tests provide an actual value.
    return file;
  }
...
function classify(
  file,
  { cwd, extensions, helperPatterns, testPatterns, sourcePatterns }
) {
  let isHelper = false;
  let isTest = false;
  let isSource = false;
