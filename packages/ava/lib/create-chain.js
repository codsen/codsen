...
function createChain(fn, defaults, meta) {
  ...
  root.after = createHookChain(
    startChain("test.after", fn, { ...defaults, type: "after" }),
    true
  );
  root.afterEach = createHookChain(
    startChain("test.afterEach", fn, { ...defaults, type: "afterEach" }),
    true
  );
  root.before = createHookChain(
    startChain("test.before", fn, { ...defaults, type: "before" }),
    false
  );
  root.beforeEach = createHookChain(
    startChain("test.beforeEach", fn, { ...defaults, type: "beforeEach" }),
    false
  );

  root.serial.after = createHookChain(
    startChain("test.after", fn, { ...defaults, serial: true, type: "after" }),
    true
  );
  root.serial.afterEach = createHookChain(
    startChain("test.afterEach", fn, {
      ...defaults,
      serial: true,
      type: "afterEach"
    }),
  ...
  // to be serial.
  root.todo = startChain("test.todo", fn, {
    ...defaults,
    type: "test",
    todo: true
  });
