...
class MiniReporter {
  constructor(options) {
    ...
    this.consumeStateChange = whileCorked(
      this.reportStream,
      whileCorked(this.lineWriter, this.consumeStateChange)
    );
  ...
  startRun(plan) {
    ...
    if (this.watching && plan.runVector > 1) {
      this.reportStream.write(
        chalk.gray.dim("\u2500".repeat(this.lineWriter.columns)) + os.EOL
      );
    }
  ...
  writeErr(evt) {
    ...
    if (evt.err.source) {
      ...
      const excerpt = codeExcerpt(evt.err.source, {
        maxWidth: this.lineWriter.columns - 2
      });
      if (excerpt) {
        this.lineWriter.writeLine();
  ...
  endRun() {
    ...
    if (
      this.filesWithMissingAvaImports.size > 0 ||
      this.filesWithoutDeclaredTests.size > 0
    ) {
      this.lineWriter.writeLine();
    }
    ...
    if (this.uncaughtExceptions.length > 0) {
      const writeLeadingLine =
        this.failures.length === 0 && this.internalErrors.length === 0;
      const writeTrailingLines =
        shouldWriteFailFastDisclaimer || this.unhandledRejections.length > 0;
