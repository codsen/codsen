...
class TapReporter {
  ...
  writeTest(evt, flags) {
    this.reportStream.write(
      supertap.test(this.prefixTitle(evt.testFile, evt.title), {
        comment: evt.logs,
  ...
  writeCrash(evt, title) {
    this.crashCount++;
    this.reportStream.write(
      supertap.test(title || evt.err.summary || evt.type, {
        comment: evt.logs,
        error: evt.err ? dumpError(evt.err) : null,
        index: ++this.i,
        passed: false,
  ...
  consumeStateChange(evt) {
    ...
    switch (evt.type) {
      ...
      case "missing-ava-import":
        ...
        this.writeCrash(
          evt,
          `No tests found in ${path.relative(
            ".",
            evt.testFile
      ...
      case "timeout":
        this.writeCrash(
          evt,
          `Exited because no new tests completed within the last ${evt.period}ms of inactivity`
        );
        break;
      ...
      case "worker-failed":
        if (!this.filesWithMissingAvaImports.has(evt.testFile)) {
          if (evt.nonZeroExitCode) {
            ...
            this.writeCrash(
              evt,
              `${path.relative(".", evt.testFile)} exited due to ${evt.signal}`
            );
          }
        }
      ...
      case "worker-finished":
        if (
          ...
          if (fileStats.declaredTests === 0) {
            this.writeCrash(
              evt,
              `No tests found in ${path.relative(".", evt.testFile)}`
            );
          } else if (!this.failFastEnabled && fileStats.remainingTests > 0) {
            this.writeCrash(
              evt,
              `${fileStats.remainingTests} ${plur(
                "test",
