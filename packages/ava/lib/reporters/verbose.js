...
class VerboseReporter {
    ...
    switch (evt.type) {
      ...
      case "internal-error":
        if (evt.testFile) {
          this.lineWriter.writeLine(
            colors.error(
              `${figures.cross} Internal error when running ${path.relative(
                ".",
                evt.testFile
              )}`
            )
      ...
      case "missing-ava-import":
        ...
        this.lineWriter.writeLine(
          colors.error(
            `${figures.cross} No tests found in ${path.relative(
              ".",
              evt.testFile
      ...
      case "uncaught-exception":
        ...
        this.lineWriter.writeLine(
          colors.title(
            `Uncaught exception in ${path.relative(".", evt.testFile)}`
          )
        );
        this.lineWriter.writeLine();
        this.writeErr(evt);
      ...
      case "unhandled-rejection":
        ...
        this.lineWriter.writeLine(
          colors.title(
            `Unhandled rejection in ${path.relative(".", evt.testFile)}`
          )
        );
        this.lineWriter.writeLine();
        this.writeErr(evt);
          ...
          if (fileStats.declaredTests === 0) {
            ...
            this.lineWriter.writeLine(
              colors.error(
                ...
                )} remaining in ${path.relative(".", evt.testFile)}`
              )
            );
          }
        }
  ...
  writeErr(evt) {
    ...
    if (evt.err.source) {
      this.lineWriter.writeLine(
        colors.errorSource(`${evt.err.source.file}:${evt.err.source.line}`)
      );
  ...
  writePendingTests(evt) {
    for (const [file, testsInFile] of evt.pendingTests) {
      ...
      this.lineWriter.writeLine(
        `${testsInFile.size} tests were pending in ${file}\n`
      );
  ...
  endRun() {
    ...
    if (!this.stats) {
      this.lineWriter.writeLine(
        colors.error(`${figures.cross} Couldn't find any files to test`)
      );
      this.lineWriter.writeLine();
      return;
    }
    ...
    if (this.matching && this.stats.selectedTests === 0) {
      this.lineWriter.writeLine(
        colors.error(`${figures.cross} Couldn't find any matching tests`)
      );
      this.lineWriter.writeLine();
      return;
    }
      ...
      this.lineWriter.writeLine(
        colors.information(
          `Ran ${currentFileCount} test ${plur(
            "file",
            currentFileCount
          )} out of ${this.stats.files} for job ${currentIndex +
            1} of ${totalRuns}`
        )
      );
      this.lineWriter.writeLine();
    ...
    const shouldWriteFailFastDisclaimer =
