...
`,
  {
    string: ["_"],
    boolean: ["fail-fast", "verbose", "serial", "tap"],
    default: conf,
    alias: {
      s: "serial"
    }
...
const cacheDir =
  conf.cacheEnabled === false
    ? uniqueTempDir()
    : path.join(projectDir, "node_modules", ".cache", "ava");
...
const precompileFull = babelPipeline.build(
  process.cwd(),
  cacheDir,
  babelConfig,
  conf.compileEnhancements === true
);

let precompileEnhancementsOnly = () => null;
if (conf.compileEnhancements) {
  precompileEnhancementsOnly = babelPipeline.build(
    projectDir,
    cacheDir,
    null,
...
const precompiled = {};
precompiled[file] = _regexpFullExtensions.test(file)
  ? precompileFull(file)
  : precompileEnhancementsOnly(file);
