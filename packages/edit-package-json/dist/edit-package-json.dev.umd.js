...
function main({ str, path, valToInsert, mode }) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (typeof withinQuotesSince !== "number" &&
            str[i] === "{" &&
            isNotEscape(str, i - 1) &&
            !replaceThisValue) {
        ...
        if (typeof withinQuotesSince !== "number" && str[i] === "]") {
            ...
            else if (withinArrayIndexes.length &&
        ...
        if (skipUntilTheFollowingIsMet.length &&
            str[i] ===
        ...
        else if ((typeof withinQuotesSince !== "number" || withinQuotesSince === i) &&
            replaceThisValue &&
            !currentlyWithinArray &&
            typeof valueStartedAt === "number") {
        ...
        if (replaceThisValue &&
            ...
            if (typeof withinQuotesSince !== "number" &&
                ...
                if (mode === "del") {
                    // 1. if del()
                    log();
                    log(`851 ${`\u001b[${33}m${`keyStartedAt`}\u001b[${39}m`} = ${JSON.stringify(keyStartedAt, null, 4)}; val = ${(currentlyWithinArray
                        ? valueStartedAt
                        : keyStartedAt) - 1}`);
                    let startingPoint = left(str, (currentlyWithinArray ? valueStartedAt : keyStartedAt) -
                        1);
