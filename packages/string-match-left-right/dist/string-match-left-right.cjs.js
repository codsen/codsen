...
function main(mode, str, position, originalWhatToMatch, originalOpts) {
  ...
  for (var _i = 0, _len = whatToMatch.length; _i < _len; _i++) {
    ...
    if (mode === 'matchRight' && stringCharacterIsAstralSurrogate.isHighSurrogate(str[_startingPosition - 1]) && stringCharacterIsAstralSurrogate.isLowSurrogate(str[_startingPosition])) {
      _startingPosition += 1;
      if (DEBUG) {
    ...
    if (existy(_found) && existy(str[_found + whatToMatch[_i].length])) {
      ...
      if (stringCharacterIsAstralSurrogate.isHighSurrogate(str[indexOfTheCharacterAfter]) && stringCharacterIsAstralSurrogate.isLowSurrogate(str[indexOfTheCharacterAfter + 1])) {
