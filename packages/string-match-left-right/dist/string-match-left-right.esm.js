...
function march(str, position, whatToMatchVal, originalOpts, special = false, getNextIdx = defaultGetNextIdx) {
  ...
  let charsToCheckCount = special ? 1 : whatToMatchVal.length; // this is the counter of real characters matched. It is not reduced
  ...
  let charsMatchedTotal = 0; // used to catch frontal false positives, where too-eager matching
  ...
  let i = position; // internal-use flag, not the same as "atLeastSomethingWasMatched":

  let somethingFound = false; // these two drive opts.firstMustMatch and opts.lastMustMatch:

  let firstCharacterMatched = false;
  let lastCharacterMatched = false; // bail early if there's whitespace in front, imagine:
  // abc important}
  //   ^
  ...
  while (str[i]) {
    ...
    const charToCompareAgainst = nextIdx > i ? whatToMatchVal[whatToMatchVal.length - charsToCheckCount] : whatToMatchVal[charsToCheckCount - 1]; // let's match
    ...
    if (!opts.i && str[i] === charToCompareAgainst || opts.i && str[i].toLowerCase() === charToCompareAgainst.toLowerCase()) {
      ...
      if (opts.maxMismatches && patience && i) {
        ...
        for (let y = 0; y <= patience; y++) { // maybe str[i] will match against next charToCompareAgainst?
...
function main(mode, str, position, originalWhatToMatch, originalOpts) {
  ...
  Array.isArray(whatToMatch) && whatToMatch.length === 1 && isStr(whatToMatch[0]) && !whatToMatch[0].trim() // [""]
      ...
      if (typeof opts.cb === "function") {
        let firstCharOutsideIndex; // matchLeft() or matchRightIncl() methods start at index "position"
        ...
        if (mode[5] === "L") {
          for (let y = startingPosition; y--;) {
            ...
            const currentChar = str[y]; // do the actual evaluation, is the current character non-whitespace/non-skiped
  ...
  for (let i = 0, len = whatToMatch.length; i < len; i++) {
    ...
    const found = march(str, startingPosition, whatToMatchVal, opts, special, i2 => mode[5] === "L" ? i2 - 1 : i2 + 1); // if march() returned positive result and it was "special" case,
