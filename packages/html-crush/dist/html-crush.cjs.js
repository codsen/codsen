...
function crush(str, originalOpts) {
  ...
  var opts = _objectSpread__default['default'](_objectSpread__default['default']({}, defaults), originalOpts); // normalize the opts.removeHTMLComments
  ...
  if (len) {
    for (var i = 0; i < len; i++) {
        ...
        if (whitespaceStartedAt !== null) {
          ...
          if (beginningOfAFile) {
            ...
            if (opts.removeLineBreaks || withinInlineStyle) { //
              ...
              if (breakToTheLeftOfFirstLetters.includes(str[i]) && stringMatchLeftRight.matchRightIncl(str, i, opts.breakToTheLeftOf)) { // maybe there was just single line break?
        ...
        if (breakToTheLeftOfFirstLetters && stringMatchLeftRight.matchRightIncl(str, i, opts.breakToTheLeftOf) && str.slice(0, i).trim() && (!str.startsWith("<![endif]", i) || !stringMatchLeftRight.matchLeft(str, i, "<!--"))) {
          ...
          if (!str[i + 1] || CHARS_BREAK_ON_THE_LEFT_OF_THEM.includes(str[i]) && !CHARS_DONT_BREAK_ON_THE_LEFT_OF_THEM.includes(str[i]) || CHARS_BREAK_ON_THE_RIGHT_OF_THEM.includes(str[i]) || !str[i].trim()) { // 1. release stage contents - now they'll be definitely deleted
            ...
            if (stageFrom !== null && stageTo !== null && (stageFrom !== stageTo || stageAdd && stageAdd.length)) {
              ...
              if (countCharactersPerLine + (_whatToAdd ? _whatToAdd.length : 0) > opts.lineLengthLimit || !(_whatToAdd === " " && stageTo === stageFrom + 1)) { // push this range only if it's not between curlies, } and {
      ...
      if (!doNothing && !beginningOfAFile && opts.removeLineBreaks && opts.lineLengthLimit && countCharactersPerLine >= opts.lineLengthLimit && stageFrom !== null && stageTo !== null && !CHARS_BREAK_ON_THE_RIGHT_OF_THEM.includes(str[i]) && !CHARS_BREAK_ON_THE_LEFT_OF_THEM.includes(str[i]) && !"/".includes(str[i])) { // two possible cases:
        ...
        if (!(countCharactersPerLine === opts.lineLengthLimit && str[i + 1] && !str[i + 1].trim())) {
          ...
          if (_whatToAdd2 === "\n" && !str[~-stageFrom].trim() && stringLeftRight.left(str, stageFrom)) {
    ...
    if (finalIndexesToDelete.current()) {
      ...
      var res = rangesApply.rApply(str, finalIndexesToDelete.current(), function (applyPercDone) {
