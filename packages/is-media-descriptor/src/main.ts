...
function isMediaD(str: string, opts?: Partial<Opts>): ResObj[] {
  ...
  if (typeof str !== "string") {
    DEV &&
      console.log(
        `isMediaD(): early exit, ${`\u001b[${31}m${`RETURN`}\u001b[${39}m`} []`,
      );
    return [];
  }
  if (!str.trim()) {
  ...
  if (str !== str.trim()) {
    let ranges = [];
    if (!str[0].trim()) {
      DEV && console.log(`traverse forward`);
    ...
    if (!str[str.length - 1].trim()) {
      DEV && console.log(`traverse backwards from the end`);
    ...
    DEV &&
      console.log(`PUSH [${ranges[0][0]}, ${ranges[ranges.length - 1][1]}]`);
    res.push({
      idxFrom: ranges[0][0],
  ...
  DEV &&
    console.log(
      `isMediaD(): ██ working non-whitespace range: [${`\u001b[${35}m${nonWhitespaceStart}\u001b[${39}m`}, ${`\u001b[${35}m${nonWhitespaceEnd}\u001b[${39}m`}]`,
    );
  ...
  if (recognisedMediaTypes.includes(resolvedStr)) {
    ...
    DEV &&
      console.log(
        `isMediaD(): whole string matched! ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`}`,
      );
    return res;
  }
  if (["only", "not"].includes(resolvedStr)) {
    DEV &&
      console.log(
        `isMediaD(): PUSH [${nonWhitespaceStart + resolvedOpts.offset}, ${
    ...
    DEV &&
      console.log(
        `isMediaD(): ${`\u001b[${36}m${`PART I.`}\u001b[${39}m`} Preliminary checks.`,
      );
      ...
      (acc, curr, idx) => {
        if (curr === ")") {
          ...
          if (!wrongOrder && acc[1] + 1 > acc[0]) {
            DEV &&
              console.log(
                `isMediaD(): set ${`\u001b[${33}m${`wrongOrder`}\u001b[${39}m`} = true`,
              );
            wrongOrder = true;
          }
    ...
    if (wrongOrder && openingBracketCount === closingBracketCount) {
      DEV &&
        console.log(
          `isMediaD(): ${`\u001b[${32}m${`PUSH`}\u001b[${39}m`} the wrong order error`,
        );
      res.push({
        idxFrom: nonWhitespaceStart + resolvedOpts.offset,
        idxTo: nonWhitespaceEnd + resolvedOpts.offset,
    ...
    DEV &&
      console.log(
        `isMediaD(): ${`\u001b[${33}m${`openingBracketCount`}\u001b[${39}m`} = ${JSON.stringify(
          openingBracketCount,
          null,
          4,
        )}`,
    ...
    DEV &&
      console.log(
        `isMediaD(): ${`\u001b[${33}m${`closingBracketCount`}\u001b[${39}m`} = ${JSON.stringify(
          closingBracketCount,
          null,
          4,
        )}`,
    ...
    if (!res.length && resolvedStr.match(/\(\s*\)/g)) {
      DEV && console.log(`empty brackets pair detected`);
      // now find out where
    ...
    if (res.length) {
      ...
      DEV &&
        console.log(
          `isMediaD(): early ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`}`,
        );
      return res;
    }
    ...
    DEV &&
      console.log(
        `isMediaD(): ${`\u001b[${36}m${`PART II.`}\u001b[${39}m`} The main loop.`,
      );
    ...
    processCommaSep(resolvedStr, {
      ...
      cb: (idxFrom: number, idxTo: number) => {
        DEV &&
          console.log(
            `isMediaD(): chunk [${idxFrom - resolvedOpts.offset}, ${
  ...
  DEV &&
    console.log(`isMediaD(): ${`\u001b[${32}m${`FINAL RETURN`}\u001b[${39}m`}`);
  DEV &&
    console.log(
      `isMediaD(): ${`\u001b[${33}m${`res`}\u001b[${39}m`} = ${JSON.stringify(
        res,
