...
function isAttrClosing(str, idxOfAttrOpening, isThisClosingIdx) {
  ...
  let lastMatchedQuotesPairsEndIsAt; // when suspected attribute name chunks end, we wipe them, but here
  ...
  let secondLastCapturedChunk; // this boolean flag signifies, was the last chunk captured after passing
  // "isThisClosingIdx":
  ...
  let lastChunkWasCapturedAfterSuspectedClosing = false; // does what it says on the tin - flips on the first instance

  let closingBracketMet = false;
  let openingBracketMet = false; // let's traverse from opening to the end of the string, then in happy
  ...
  for (let i = idxOfAttrOpening, len = str.length; i < len; i++) {
    ...
    i >= isThisClosingIdx) { // ███████████████████████████████████████ E1
      ...
      const E1 = i !== isThisClosingIdx || guaranteedAttrStartsAtX(str, right(str, isThisClosingIdx)) || `/>`.includes(str[right(str, i)]); // ███████████████████████████████████████ E2
      //
      //
      ...
      const E2 = !(i > isThisClosingIdx && str[idxOfAttrOpening] === str[isThisClosingIdx] && str[idxOfAttrOpening] === str[i] && // rule out cases where plausible attribute starts:
      // <img class="so-called "alt"!' border='10'/>
      //            ^          ^   ^
      //        start          |    \
      ...
      const E31 = // or a proper recognised attribute follows:
      // <img alt="so-called "artists"class='yo'/>
      //          ^                  ^
      ...
      const E32 = // or the last chunk is a known attribute name:
      // <img class="so-called "alt"!' border='10'/>
      //            ^          ^
      ...
      const E33 = chunkStartsAt && chunkStartsAt < i && str[chunkStartsAt - 1] && !str[chunkStartsAt - 1].trim() && // and whole chunk is a plausible attribute name
      Array.from(str.slice(chunkStartsAt, i).trim()).every(char => isAttrNameChar(char)) && // known opening and suspected closing are both singles or doubles
      ...
      const E41 = // either it's a tag ending and we're at the suspected quote
      `/>`.includes(str[right(str, i)]) && i === isThisClosingIdx;
      const E42 = // or next character is suitable for a tag name:
      isAttrNameChar(str[right(str, i)]);
      const E43 = // or in case of:
      // <img class="so-called "alt"!' border='10'/>
      //            ^          ^
      //          start      suspected
      //
      ...
      const E5 = // it's not a double-wrapped attribute value:
      //
    ...
    if (str[i].trim() && !chunkStartsAt) { // <img alt="so-called "artists"!' class='yo'/>
      ...
      if (`'"`.includes(str[i]) && quotesCount.get(`matchedPairs`) === 0 && totalQuotesCount === 3 && str[idxOfAttrOpening] === str[i] && allHtmlAttribs.has(lastCapturedChunk) && !`'"`.includes(str[right(str, i)])) {
        ...
        const A23 = str.slice(lastQuoteAt + 1, i).trim().split(/\s+/).every(chunk => allHtmlAttribs.has(chunk)); // <div style="float:'left"">z</div>
        //            ^           ^
        //          start      we're here
    ...
    !(lastCapturedChunk && secondLastCapturedChunk && secondLastCapturedChunk.trim().endsWith(":"))) { // rules:
      ...
      const R3 = allHtmlAttribs.has(str.slice(idxOfAttrOpening + 1, isThisClosingIdx).trim()); // that quote we suspected as closing, is from an opening-closing
      // set on another attribute:

      const R4 = !xBeforeYOnTheRight(str, i + 1, str[isThisClosingIdx], makeTheQuoteOpposite(str[isThisClosingIdx])); // const R5 = plausibleAttrStartsAtX(str, start) // consider:
      // <z alt"href' www'/>
      //       ^    ^
      //    start   suspected ending
    ...
    if (i > isThisClosingIdx) { // if current quote matches the opening
      ...
      if (openingQuote && str[i] === openingQuote) { // we want to return false as default...
        ...
        const Y2 = lastQuoteAt === isThisClosingIdx; // ensure there's some content between suspected and "here":
      ...
      if (str[i] === "/" || str[i] === ">" || str[i] === "<") { // happy path scenario
        ...
        const R0 = // opening matches closing
        ...
        const R11 = quotesCount.get(`matchedPairs`) < 2; // at least it's not a recognised attribute name on the left:

        const attrNameCharsChunkOnTheLeft = findAttrNameCharsChunkOnTheLeft(str, i);
        const R12 = (!attrNameCharsChunkOnTheLeft || !allHtmlAttribs.has(attrNameCharsChunkOnTheLeft)) && ( // avoid cases where multiple pairs of mismatching quotes were matched
        ...
        const R2 = totalQuotesCount < 3 || // there's only two quotes mismatching:
        ...
        return (// happy path - known opening matched suspected closing and
          ...
          R2 && ( // also, protection against cases like:
          // <z bbb"c" ddd'e>
          //       ^      ^
          //   start     suspected
          //
...
export { isAttrClosing, version$1 as version };
