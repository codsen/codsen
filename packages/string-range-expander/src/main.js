...
function expander(originalOpts) {
  ...
  if (
    ...
    for (let i = from; i--; ) {
      ...
      if (
        ...
        from = i + 2;
        console.log(
          `157 SET ${`\u001b[${33}m${`from`}\u001b[${39}m`} = ${from}, BREAK`
        );
        break;
      } else if (i === 0) {
        from = 1;
        console.log(
          `163 SET ${`\u001b[${33}m${`from`}\u001b[${39}m`} = ${from}`
        );
        break;
      }
    }
  ...
  console.log(`220 RETURN: [${from}, ${to}]`);
  return [from, to];
}
