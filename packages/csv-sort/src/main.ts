...
function sort(input: string): Res {
  ...
  let stateColumnsContainingSameValueEverywhere = [];

  // used for 2D trimming:
  let indexAtWhichEmptyCellsStart: number | null = null;

  for (let i = content.length - 1; i >= 0; i--) {
    DEV && console.log(`content[${i}] = ${content[i]}`);
    if (!schema.length) {
      // prevention against last blank row:
      /* c8 ignore next */
      if (content[i].length !== 1 || content[i][0] !== "") {
        for (let y = 0, len = content[i].length; y < len; y++) {
          const cellType = findType(content[i][y].trim());
          schema.push(cellType);
          if (indexAtWhichEmptyCellsStart === null && cellType === "empty") {
      ...
      if (i === 0) {
        ...
        stateHeaderRowPresent = content[i].every((el) => {
          const cellType = findType(el);
          return cellType === "text" || cellType === "empty";
        });
      ...
      for (let y = 0, len = content[i].length; y < len; y++) {
        ...
        // checking schema
        /* c8 ignore next */
        if (cellType !== schema[y] && !stateHeaderRowPresent) {
          let toAdd = cellType;
          /* c8 ignore next */
  ...
  // ----------------
  let numericSchemaColumns: number[] = [];
  ...
  let traverseUpToThisIndexAtTheTop = stateHeaderRowPresent ? 1 : 0;
  ...
  if (numericSchemaColumns.length === 1) {
    ...
    throw new Error(
      'csv-sort/sort(): [THROW_ID_02] Your CSV file does not contain numeric-only columns and computer was not able to detect the "Balance" column!',
    );
  } else {
    ...
    for (
      ...
      let suspectedBalanceColumnsIndexNumber =
    ...
    if (potentialBalanceColumnIndexesList.length === 1) {
      ...
      throw new Error(
        'csv-sort/sort(): [THROW_ID_03] The computer can\'t find the "Balance" column! It saw some numeric-only columns, but they all seem to have certain rows with the same values as rows right below/above them!',
      );
    } else {
  ...
  if (!balanceColumnIndex) {
    throw new Error(
      "csv-sort/sort(): [THROW_ID_04] Sadly computer couldn't find its way in this CSV and had to stop working on it.",
    );
  }
  ...
  DEV &&
    console.log(
      `after push ${`\u001b[${33}m${`resContent`}\u001b[${39}m`} = ${JSON.stringify(
        resContent,
        null,
        4,
      )}`,
  ...
  let bottom = stateHeaderRowPresent ? 1 : 0;
  for (let y = content.length - 2; y >= bottom; y--) {
    ...
    for (
      ...
      DEV &&
        console.log(
          `\u001b[${90}m${`=============== suspected row: ${JSON.stringify(
      ...
      if (!usedUpRows.includes(suspectedRowsIndex)) {
        ...
        for (
          ...
          DEV &&
            console.log(
              `\u001b[${90}m${`--------------- suspectedColIndex = ${suspectedColIndex} ---------------`}\u001b[${39}m`,
            );
          let diffVal = null;
          if (
            content[suspectedRowsIndex][
            ...
            DEV &&
              console.log(
                `SET ${`\u001b[${33}m${`diffVal`}\u001b[${39}m`} = ${JSON.stringify(
                  diffVal,
                  null,
                  4,
                )}`,
          ...
          if (content[suspectedRowsIndex][balanceColumnIndex] !== "") {
            ...
            DEV &&
              console.log(
                `SET ${`\u001b[${33}m${`totalVal`}\u001b[${39}m`} = ${JSON.stringify(
                  totalVal,
                  null,
                  4,
                )}`,
          ...
          if (resContent[0][balanceColumnIndex] !== "") {
            ...
            DEV &&
              console.log(
                `SET ${`\u001b[${33}m${`topmostResContentBalance`}\u001b[${39}m`} = ${JSON.stringify(
          ...
          DEV &&
            console.log(
              `${`\u001b[${33}m${`diffVal`}\u001b[${39}m`} = ${JSON.stringify(
                diffVal,
                null,
                4,
              )}`,
          ...
          if (
            ...
            DEV && console.log(`ADD THIS ROW ABOVE EVERYTHING`);
          ...
          DEV &&
            console.log(
              `${`\u001b[${33}m${`thisRowIsDone`}\u001b[${39}m`} = ${JSON.stringify(
                thisRowIsDone,
                null,
                4,
              )}`,
    ...
    DEV &&
      console.log(
        `${`\u001b[${32}m${`██`}\u001b[${39}m`} ENDING \u001b[${33}m${`resContent`}\u001b[${39}m = ${JSON.stringify(
          resContent,
          null,
          4,
        )}`,
