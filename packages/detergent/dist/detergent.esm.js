...
function processCharacter(str, opts, rangesArr, i, y, offsetBy, brClosingBracketIndexesArr, state, applicableOpts, endOfLineVal) {
  const len = str.length; // console.log(`075 received endOfLineVal = ${JSON.stringify(endOfLineVal, null, 0)}`);
  ...
  if (/[\uD800-\uDFFF]/g.test(str[i]) && !(str.charCodeAt(i + 1) >= 0xdc00 && str.charCodeAt(i + 1) <= 0xdfff || str.charCodeAt(i - 1) >= 0xd800 && str.charCodeAt(i - 1) <= 0xdbff)) {
    ...
    const charcode = str[i].charCodeAt(0); // Filter ASCII
    ...
    if (charcode < 127) {
      ...
      if (charcode < 32) {
        if (charcode < 9) {
          ...
          if (opts.removeLineBreaks) {
            ...
            rangesArr.push(startingIdx, endingIdx, `<br${opts.useXHTML ? "/" : ""}>${whatToInsert}`); // skip the \n that follows
      ...
      if (charcode > 126 && charcode < 160) {
        ...
        if (!opts.removeWidows) {
          ...
          let calculatedValue = " "; // const charOnTheLeft = leftStopAtRawNbsp(str, i);

          const charOnTheLeft = left(str, i); // const charOnTheRight = rightStopAtRawNbsp(str, calculatedTo - 1);
        ...
        if (str[i - 1] && !str[i - 1].trim() && str[y].trim()) {
          ...
          if (str[i - 2] && isNumber(str[i - 2]) && isNumber(str[y])) {
            ...
            if (opts.addMissingSpaces) {
              let whatToAdd = " "; // imagine case "10am&nbsp;&ndash;11am" - we're adding space
...
function det(str, inputOpts) {
  ...
  const brClosingBracketIndexesArr = []; // We need to track what actions need to be done. Each action (a range) is
    ...
    const cb = ({
      ...
      if (isNum(tag.lastOpeningBracketAt) && isNum(tag.lastClosingBracketAt) && tag.lastOpeningBracketAt < tag.lastClosingBracketAt || tag.slashPresent) {
        ...
        if (opts.stripHtml && !opts.stripHtmlButIgnoreTags.includes(tag.name.toLowerCase())) {
          ...
          if (Array.isArray(opts.stripHtmlAddNewLine) && opts.stripHtmlAddNewLine.length && opts.stripHtmlAddNewLine.some(tagName => tagName.startsWith("/") && // present slash will be reported for both frontal and
