          ...
          if (str[whatsOnTheLeft] === "&" && (!str[i] || str[i] !== ";")) {
            ...
            if (str.slice(whatsOnTheLeft + 1, i).trim().length > 1) {
              ...
              if (situation.probablyNumeric) {
                ...
                } else if (potentialEntity.length < maxLength + 2 && ((temp = [...allNamedEntitiesSetOnly].filter(curr => leven_1(curr, potentialEntity) === 1)) && temp.length || (temp = [...allNamedEntitiesSetOnly].filter(curr => leven_1(curr, potentialEntity) === 2 && curr.length > 3)) && temp.length)) {
                  if (temp.length === 1) {
                    [tempEnt] = temp;
                    rangesArr2.push({
                      ruleName: `bad-named-html-entity-malformed-${tempEnt}`,
