  ...
  for (let i = 0, len = str.length; i < len; i++) {
    ...
    if (str[i] === '"') {
      ...
      if (ignoreCommasThatFollow && str[i + 1] === '"') {
        ...
        const newElem = str.slice(colStarts, i); // if the element contains only empty space,
    ...
    if (i + 1 === len) {
      ...
      if (str[i] !== '"') {
        const newElem = str.slice(colStarts, i + 1); // if the element contains only empty space,
