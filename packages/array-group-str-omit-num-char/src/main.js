...
function groupStr(originalArr, originalOpts) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (!digitChunks) {
      ...
      if (compiledObj.hasOwnProperty(wildcarded)) {
        ...
        console.log(
          `133 creating entry for "${wildcarded}"; compiledObj[wildcarded] = ${JSON.stringify(
            compiledObj[wildcarded],
            null,
            4
          )}`
  ...
  console.log();
  console.log(
    `145 FINAL ${`\u001b[${33}m${`compiledObj`}\u001b[${39}m`} = ${JSON.stringify(
      compiledObj,
      null,
      4
    )}\n`
  ...
  Object.keys(compiledObj).forEach(key => {
    ...
    console.log(
      `158 PROCESSING compiledObj key: ${JSON.stringify(key, null, 4)}`
    );
