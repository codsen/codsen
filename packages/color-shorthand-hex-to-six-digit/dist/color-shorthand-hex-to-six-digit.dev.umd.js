...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.colorShorthandHexToSixDigit = factory());
  ...
  function conv(originalInput) {
    ...
    var input = lodash_clonedeep(originalInput); // f's
    // ====================
    ...
    if (typeof originalInput === "string") {
      input = input.replace(hexColorRegex(), toFullHex);
    } else if (Array.isArray(input)) {
      for (var i = 0, len = input.length; i < len; i++) {
        input[i] = conv(input[i]);
      }
    } else if (lodash_isplainobject(originalInput)) {
