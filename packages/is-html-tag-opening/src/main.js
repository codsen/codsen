...
function isSpace(char) {
  return char === undefined || (typeof char === "string" && char.trim() === "");
}

function isOpening(str, idx = 0) {
  console.log(
    `129 ${`\u001b[${33}m${`idx`}\u001b[${39}m`} = ${`\u001b[${31}m${idx}\u001b[${39}m`}`
  );
  console.log(`131 idx = ${idx}`);
  ...
  if (r1.test(whatToTest)) {
    console.log(`149 ${`\u001b[${31}m${`R1`}\u001b[${39}m`} passed`);
    passed = true;
  } else if (r2.test(whatToTest)) {
    console.log(`152 ${`\u001b[${31}m${`R2`}\u001b[${39}m`} passed`);
    passed = true;
  } else if (r3.test(whatToTest)) {
    console.log(`155 ${`\u001b[${31}m${`R3`}\u001b[${39}m`} passed`);
    passed = true;
  } else if (r4.test(whatToTest)) {
    console.log(`158 ${`\u001b[${31}m${`R4`}\u001b[${39}m`} passed`);
    passed = true;
  } else if (
    str[idx] === "<" &&
    str[idx + 1] &&
    str[idx + 1].trim().length &&
    matchRight(str, idx, knownHtmlTags, { cb: isSpace })
  ) {
    passed = true;
  }
  const res = isStr(str) && idx < str.length && passed;
  console.log(`167 return ${`\u001b[${36}m${res}\u001b[${39}m`}`);
  return res;
}
