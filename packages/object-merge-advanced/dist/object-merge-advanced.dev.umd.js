...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.objectMergeAdvanced = factory());
  ...
  */
  ...
  function mergeAdvanced(infoObj, input1orig, input2orig) {
    ...
    var defaults = {
      cb: null,
      // cb(input1, input2, result)
      mergeObjectsOnlyWhenKeysetMatches: true,
      // otherwise, concatenation will be preferred
      ignoreKeys: [],
      hardMergeKeys: [],
      hardArrayConcatKeys: [],
    ...
    opts.hardMergeKeys = arrayiffyString(opts.hardMergeKeys); // hardMergeKeys: '*' <===> hardMergeEverything === true
    ...
    var i2 = isArr$2(input2orig) || isObj(input2orig) ? lodash_clonedeep(input2orig) : input2orig; // // if the unidirectional merging is set, that's a quick ending because the values
    // // don't matter
      ...
      if (nonEmpty(i1)) {
        ...
        if (isArr$2(i2) && nonEmpty(i2)) {
          ...
          for (var index = 0, len = Math.max(i1.length, i2.length); index < len; index++) {
            ...
            currPath = infoObj.path.length ? "".concat(infoObj.path, ".").concat(index) : "".concat(index); // calculate the merge outcome:
      ...
      if (nonEmpty(i1)) {
        ...
        if (isObj(i2)) {
          ...
          Object.keys(i2).forEach(function (key) {
            // calculate current path:
            currPath = infoObj.path && infoObj.path.length ? "".concat(infoObj.path, ".").concat(key) : "".concat(key); // calculate the merge outcome:
