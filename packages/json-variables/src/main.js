...
import get from 'posthtml-ast-get-values-by-key'
...
function trimIfString(something) { return isStr(something) ? something.trim() : something }
...
function containsHeadsOrTails(str, opts) {
  ...
  if (
    ...
    (isStr(opts.headsNoWrap) && (opts.headsNoWrap.length > 0) && str.includes(opts.headsNoWrap)) ||
    (isStr(opts.tailsNoWrap) && (opts.tailsNoWrap.length > 0) && str.includes(opts.tailsNoWrap))
  ) {
    return true
  }
  return false
...
function resolveString(input, string, path, opts, incomingBreadCrumbPath = []) {
      ...
      if (varName.length === 0) {
        ...
        if (containsHeadsOrTails(resolvedValue, opts)) {
          slices.push(
            ...
            obj.tailsEndAt, // replace upto index
            wrap(
              resolveString( // replacement value    <--------- R E C U R S I O N
                input,
                resolvedValue,
                newPath,
                opts,
  ...
  if (
    ...
    (replaceSlicesArr(string, [foundHeadsAndTails[0].headsStartAt, foundHeadsAndTails[0].tailsEndAt]).trim() === '')
  ) {
...
function jsonVariables(inputOriginal, originalOpts = {}) {
  ...
  if (!isObj(inputOriginal)) {
    throw new TypeError(`json-variables/jsonVariables(): [THROW_ID_02] Alas! The input must be a plain object! Currently it's: ${Array.isArray(inputOriginal) ? 'array' : typeof inputOriginal}`)
  }
  if (!isObj(originalOpts)) {
    throw new TypeError(`json-variables/jsonVariables(): [THROW_ID_03] Alas! An Optional Options Object must be a plain object! Currently it's: ${Array.isArray(originalOpts) ? 'array' : typeof originalOpts}`)
  }
  ...
  if (opts.dontWrapVars.length > 0 && !opts.dontWrapVars.every((el, idx) => {
    ...
    throw new Error(`json-variables/jsonVariables(): [THROW_ID_05] Alas! All variable names set in opts.dontWrapVars should be of a string type. Computer detected a value "${culpritVal}" at index ${culpritIndex}, which is not string but ${Array.isArray(culpritVal) ? 'array' : typeof culpritVal}!`)
  }
  ...
  return traverse(input, (key, val, innerObj) => {
    ...
    if (
      (
        (opts.heads.length !== 0) && (trimIfString(current) === trimIfString(opts.heads))
      ) ||
      (
        (opts.tails.length !== 0) && (trimIfString(current) === trimIfString(opts.tails))
      ) ||
      (
        (opts.headsNoWrap.length !== 0) && (trimIfString(current) === trimIfString(opts.headsNoWrap))
      ) ||
      (
        (opts.tailsNoWrap.length !== 0) && (trimIfString(current) === trimIfString(opts.tailsNoWrap))
      )
    ) {
      if (!opts.noSingleMarkers) {
        return current
      }
      throw new Error(`json-variables/jsonVariables(): [THROW_ID_16] Alas! While processing the input, we stumbled upon ${trimIfString(current)} which is equal to ${
        trimIfString(current) === trimIfString(opts.heads) ? 'heads' : ''
      }${
        trimIfString(current) === trimIfString(opts.tails) ? 'tails' : ''
      }${
        (isStr(opts.headsNoWrap) && (trimIfString(current) === trimIfString(opts.headsNoWrap))) ? 'headsNoWrap' : ''
      }${
        (isStr(opts.tailsNoWrap) && (trimIfString(current) === trimIfString(opts.tailsNoWrap))) ? 'tailsNoWrap' : ''
