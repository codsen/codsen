...
function seq(
  ...
  while (i < args.length) {
    ...
    if (
      ...
      DEV &&
        console.log(
          `410 SET whattsOnTheSide = ${whattsOnTheSide} (${
      ...
      DEV &&
        console.log(
          `425 ██ ${`\u001b[${33}m${`temp`}\u001b[${39}m`} = ${JSON.stringify(
            temp,
            null,
            4,
          )}`,
      ...
      if (
        ...
        DEV &&
          console.log(
            `441 ${`\u001b[${32}m${`SET`}\u001b[${39}m`} ${`\u001b[${33}m${`satiated`}\u001b[${39}m`} = ${JSON.stringify(
              satiated,
              null,
              4,
            )}`,
        ...
        DEV &&
          console.log(
            `452 ${`\u001b[${32}m${`SET`}\u001b[${39}m`} ${`\u001b[${33}m${`i`}\u001b[${39}m`} = ${JSON.stringify(
              i,
              null,
              4,
            )}`,
      ...
      DEV &&
        console.log(
          `462 ${`\u001b[${33}m${`i`}\u001b[${39}m`} = ${JSON.stringify(
            i,
            null,
            4,
          )}`,
      ...
      if (
        ...
        DEV &&
          console.log(`478 push gap [${lastFinding + 1}, ${whattsOnTheSide}]`);
        ...
        DEV &&
          console.log(
            `487 unshift gap [${whattsOnTheSide + 1}, ${lastFinding}]`,
          );
        gaps.unshift([whattsOnTheSide + 1, lastFinding]);
      }
      DEV &&
        console.log(`492 ${`\u001b[${32}m${value} MATCHED!\u001b[${39}m`}`);
      ...
      DEV && console.log(`510 SET lastFinding = ${lastFinding}`);
    } else if (optional) {
      DEV &&
        console.log(
          `514 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because it was optional`,
        );
      i += 1;
    } else if (satiated) {
      DEV &&
        console.log(
          `520 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because "satiated" is on`,
        );
      i += 1;
      satiated = undefined;
    } else {
      DEV && console.log(`525 RETURN null`);
      return null;
    }
  }
  DEV &&
    console.log(
      `531 ${`\u001b[${34}m${`███████████████████████████████████████ seq() stops looping`}\u001b[${39}m`}`,
    );
  DEV && console.log(`533 FINAL gaps = ${JSON.stringify(gaps, null, 4)}`);

  // if all arguments in sequence were empty strings, we return falsy null:
  if (leftmostChar === undefined || rightmostChar === undefined) {
    DEV && console.log(`537 RETURN ${`\u001b[${33}m${`null`}\u001b[${39}m`}`);
    return null;
  }

  DEV &&
    console.log(
      `543 RETURN ${`\u001b[${33}m${JSON.stringify(
        {
          gaps,
          leftmostChar,
          rightmostChar,
...
function leftSeq(str: string, idx: number, ...args: any[]): SeqOutput | null {
  ...
  DEV &&
    console.log(
      `602 leftSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4,
      )}`,
  ...
  DEV && console.log(`609 leftSeq() calling seq()`);
...
function rightSeq(str: string, idx: number, ...args: any[]): SeqOutput | null {
  ...
  DEV &&
    console.log(
      `630 rightSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4,
      )}`,
    );
  DEV && console.log(`636 rightSeq() calling seq()`);
...
function chomp(
  ...
  if (
    ...
    DEV && console.log(`694 there's no space to go further in this direction`);
    return null;
  }
  ...
  DEV &&
    console.log(
      `704 ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4,
  ...
  do {
    ...
    DEV &&
      console.log(
        `734 ${`\u001b[${36}m${`lastRes = ${JSON.stringify(
          lastRes,
          null,
          4,
        )}`}\u001b[${39}m`}`,
    ...
    if (lastRes !== null) {
      ...
      DEV &&
        console.log(
          `745 ${`\u001b[${36}m${`another sequence; confirmed! Now set `}\u001b[${39}m`} ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${JSON.stringify(
            lastIdx,
            null,
            4,
          )};`,
  ...
  DEV &&
    console.log(`762 ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${lastIdx}`);
  ...
  if (direction === "right") {
    ...
    if (str[lastIdx]?.trim()) {
      ...
      DEV && console.log(`784 RETURN ${lastIdx}`);
      return lastIdx;
    }
    ...
    DEV &&
      console.log(
        `792 SET ${`\u001b[${33}m${`whatsOnTheRight`}\u001b[${39}m`} = ${whatsOnTheRight}`,
      );
    if (!opts || opts.mode === 0) {
      ...
      DEV && console.log(`829 RETURN ${lastIdx}`);
      return lastIdx;
    } else if (opts.mode === 2) {
      ...
      DEV &&
        console.log(
          `836 ${`\u001b[${33}m${`remainderString`}\u001b[${39}m`} = ${JSON.stringify(
            remainderString,
            null,
            4,
          )}`,
      ...
      if (
        ...
        for (let y = lastIdx, len = str.length; y < len; y++) {
          if (str[y].trim() || `\n\r`.includes(str[y])) {
            DEV && console.log(`850 RETURN ${y}`);
            return y;
          }
        }
      }
      // ELSE, last but not least, chomp to the end:
      DEV && console.log(`856 RETURN ${str.length}`);
      return str.length;
    }
    // ELSE - mode 3

    // mode 3 is an aggro chomp - will chump all whitespace
    DEV && console.log(`862 RETURN ${whatsOnTheRight || str.length}`);
  ...
  if (str[lastIdx] && str[~-lastIdx]?.trim()) {
    ...
    DEV && console.log(`884 RETURN ${lastIdx}`);
    return lastIdx;
  }
  ...
  DEV &&
    console.log(
      `893 SET ${`\u001b[${33}m${`whatsOnTheLeft`}\u001b[${39}m`} = ${whatsOnTheLeft}`,
    );
  DEV &&
    console.log(
      `897 FIY, ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${lastIdx}`,
    );
  if (!opts || opts.mode === 0) {
    DEV && console.log(`900`);
    ...
    if (
      ...
      DEV &&
        console.log(
          `917 ${`\u001b[${36}m${`loop backwards from ${lastIdx}`}\u001b[${39}m`}`,
        );
      // if there are line break characters between current "lastIdx" we're on
      // and the first non-whitespace character on the right
      for (let y = lastIdx; y--; ) {
        ...
        if (`\n\r`.includes(str[y]) || str[y].trim()) {
          DEV && console.log(`931 RETURN ${y + 1 + (str[y].trim() ? 1 : 0)}`);
          return y + 1 + (str[y].trim() ? 1 : 0);
        }
      }
    }
    // ELSE
    DEV && console.log(`937 ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`} 0`);
    return 0;
  }
  if (opts.mode === 1) {
    // mode 1 doesn't touch the whitespace, so it's quick:
    DEV && console.log(`942 RETURN ${lastIdx}`);
    return lastIdx;
  }
  if (opts.mode === 2) {
    // mode 2 hungrily chomps all whitespace except newlines
    ...
    if (
      ...
      for (let y = lastIdx; y--; ) {
        if (str[y].trim() || `\n\r`.includes(str[y])) {
          DEV && console.log(`956 RETURN ${y + 1}`);
          return y + 1;
        }
      }
    }
    // ELSE, last but not least, chomp to the end:
    DEV && console.log(`962 RETURN 0`);
    return 0;
  }
  // ELSE - mode 3
  ...
  DEV &&
    console.log(
      `970 RETURN ${whatsOnTheLeft !== null ? whatsOnTheLeft + 1 : 0}`,
    );
...
function chompLeft(str: string, idx: number, ...args: any[]): number | null {
  DEV &&
    console.log(
      `1012 chompLeft(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
        args,
        null,
        4,
      )}`,
  ...
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    DEV && console.log(`1020 return null because there's nothing to match`);
    return null;
  }
  DEV && console.log(`1023 chompLeft()`);
  ...
  if (isObj(args[0])) {
    ...
    DEV && console.log(`1056 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("left", str, idx, opts, args.slice(1));
  }
  if (!isStr(args[0])) {
    DEV &&
      console.log(`1061 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
    return chomp("left", str, idx, defaults, args.slice(1));
  }
  // ELSE
  // all arguments are values to match, first element is not options object
  DEV && console.log(`1066 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
...
function chompRight(str: string, idx: number, ...args: any[]): number | null {
  DEV &&
    console.log(
      `1099 chompRight(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
        args,
        null,
        4,
      )}`,
  ...
  // if there are no arguments, null
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    DEV && console.log(`1107 return null because there's nothing to match`);
    return null;
  }
  DEV && console.log(`1110 chompRight()`);
  ...
  if (isObj(args[0])) {
    ...
    DEV && console.log(`1139 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("right", str, idx, opts, args.slice(1));
  }
  if (!isStr(args[0])) {
    DEV &&
      console.log(`1144 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
    return chomp("right", str, idx, defaults, args.slice(1));
  }
  // ELSE
  // all arguments are values to match, first element is not options object
  DEV && console.log(`1149 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
