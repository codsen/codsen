...
function seq(
  ...
  DEV && console.log(`325 seq() called:`);
  if (typeof str !== "string" || !str.length) {
    return null;
  }
  ...
  if (
    ...
    DEV && console.log(`337 RETURN null`);
    return null;
  }
  ...
  let lastFinding = idx;
  DEV &&
    console.log(`346 Set lastFinding = ${lastFinding}. Starting the loop.`);
  ...
  while (i < args.length) {
    DEV &&
      console.log(
        `362 ${`\u001b[${34}m${`███████████████████████████████████████ seq() looping ${args[i]}`}\u001b[${39}m`}; i = ${i}`
      );
    if (!isStr(args[i]) || !(args[i] as string).length) {
      DEV &&
        console.log(
          `367 continue because ${JSON.stringify(
            args[i],
            null,
            4
    ...
    DEV &&
      console.log(
        `378 ${`\u001b[${36}m${`============= args[${i}]=${args[i]}`}\u001b[${39}m`}`
      );
    let { value, optional, hungry } = separateValueFromFlags(args[i]);
    DEV &&
      console.log(
        `383 ${`\u001b[${33}m${`value`}\u001b[${39}m`} = ${JSON.stringify(
          value,
          null,
          4
    ...
    DEV &&
      console.log(
        `402 ██ ${`\u001b[${33}m${`whattsOnTheSide`}\u001b[${39}m`} = ${JSON.stringify(
          whattsOnTheSide,
          null,
          4
        )}`
    ...
    if (
      ...
      DEV &&
        console.log(
          `415 SET whattsOnTheSide = ${whattsOnTheSide} (${
      ...
      DEV &&
        console.log(
          `430 ██ ${`\u001b[${33}m${`temp`}\u001b[${39}m`} = ${JSON.stringify(
            temp,
            null,
            4
          )}`
      ...
      if (
        ...
        DEV &&
          console.log(
            `446 ${`\u001b[${32}m${`SET`}\u001b[${39}m`} ${`\u001b[${33}m${`satiated`}\u001b[${39}m`} = ${JSON.stringify(
              satiated,
              null,
              4
            )}`
        ...
        DEV &&
          console.log(
            `457 ${`\u001b[${32}m${`SET`}\u001b[${39}m`} ${`\u001b[${33}m${`i`}\u001b[${39}m`} = ${JSON.stringify(
              i,
              null,
              4
            )}`
      ...
      DEV &&
        console.log(
          `467 ${`\u001b[${33}m${`i`}\u001b[${39}m`} = ${JSON.stringify(
            i,
            null,
            4
          )}`
      ...
      if (
        ...
        DEV &&
          console.log(`483 push gap [${lastFinding + 1}, ${whattsOnTheSide}]`);
        ...
        DEV &&
          console.log(
            `492 unshift gap [${whattsOnTheSide + 1}, ${lastFinding}]`
          );
        gaps.unshift([whattsOnTheSide + 1, lastFinding]);
      }
      DEV &&
        console.log(`497 ${`\u001b[${32}m${value} MATCHED!\u001b[${39}m`}`);
      ...
      DEV && console.log(`515 SET lastFinding = ${lastFinding}`);
    } else if (optional) {
      DEV &&
        console.log(
          `519 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because it was optional`
        );
      i += 1;
      continue;
    } else if (satiated) {
      DEV &&
        console.log(
          `526 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because "satiated" is on`
        );
      i += 1;
      satiated = undefined;
      continue;
    } else {
      DEV && console.log(`532 RETURN null`);
      return null;
    }
  }
  DEV &&
    console.log(
      `538 ${`\u001b[${34}m${`███████████████████████████████████████ seq() stops looping`}\u001b[${39}m`}`
    );
  DEV && console.log(`540 FINAL gaps = ${JSON.stringify(gaps, null, 4)}`);

  // if all arguments in sequence were empty strings, we return falsey null:
  if (leftmostChar === undefined || rightmostChar === undefined) {
    DEV && console.log(`544 RETURN ${`\u001b[${33}m${`null`}\u001b[${39}m`}`);
    return null;
  }

  DEV &&
    console.log(
      `550 RETURN ${`\u001b[${33}m${JSON.stringify(
        {
          gaps,
          leftmostChar,
          rightmostChar,
...
function leftSeq(str: string, idx: number, ...args: any[]): SeqOutput | null {
  ...
  DEV &&
    console.log(
      `610 leftSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4
      )}`
  ...
  DEV && console.log(`617 leftSeq() calling seq()`);
...
function rightSeq(str: string, idx: number, ...args: any[]): SeqOutput | null {
  ...
  DEV &&
    console.log(
      `639 rightSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4
      )}`
    );
  DEV && console.log(`645 rightSeq() calling seq()`);
...
function chomp(
  ...
  if (
    ...
    DEV && console.log(`703 there's no space to go further in this direction`);
    return null;
  }
  ...
  DEV &&
    console.log(
      `713 ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
        opts,
        null,
        4
  ...
  do {
    ...
    DEV &&
      console.log(
        `743 ${`\u001b[${36}m${`lastRes = ${JSON.stringify(
          lastRes,
          null,
          4
        )}`}\u001b[${39}m`}`
    ...
    if (lastRes !== null) {
      ...
      DEV &&
        console.log(
          `754 ${`\u001b[${36}m${`another sequence; confirmed! Now set `}\u001b[${39}m`} ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${JSON.stringify(
            lastIdx,
            null,
            4
          )};`
  ...
  DEV &&
    console.log(`771 ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${lastIdx}`);
  ...
  if (direction === "right") {
    ...
    if (str[lastIdx]?.trim()) {
      ...
      DEV && console.log(`793 RETURN ${lastIdx}`);
      return lastIdx;
    }
    ...
    DEV &&
      console.log(
        `801 SET ${`\u001b[${33}m${`whatsOnTheRight`}\u001b[${39}m`} = ${whatsOnTheRight}`
      );
    if (!opts || opts.mode === 0) {
      ...
      DEV && console.log(`838 RETURN ${lastIdx}`);
      return lastIdx;
    } else if (opts.mode === 2) {
      ...
      DEV &&
        console.log(
          `845 ${`\u001b[${33}m${`remainderString`}\u001b[${39}m`} = ${JSON.stringify(
            remainderString,
            null,
            4
          )}`
      ...
      if (
        ...
        for (let y = lastIdx, len = str.length; y < len; y++) {
          if (str[y].trim() || `\n\r`.includes(str[y])) {
            DEV && console.log(`859 RETURN ${y}`);
            return y;
          }
        }
      }
      // ELSE, last but not least, chomp to the end:
      DEV && console.log(`865 RETURN ${str.length}`);
      return str.length;
    }
    // ELSE - mode 3

    // mode 3 is an aggro chomp - will chump all whitespace
    DEV && console.log(`871 RETURN ${whatsOnTheRight || str.length}`);
  ...
  DEV &&
    console.log(
      `902 SET ${`\u001b[${33}m${`whatsOnTheLeft`}\u001b[${39}m`} = ${whatsOnTheLeft}`
    );
  DEV &&
    console.log(
      `906 FIY, ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${lastIdx}`
    );
  if (!opts || opts.mode === 0) {
    DEV && console.log(`909`);
    ...
    if (
      ...
      DEV &&
        console.log(
          `926 ${`\u001b[${36}m${`loop backwards from ${lastIdx}`}\u001b[${39}m`}`
        );
      // if there are line break characters between current "lastIdx" we're on
      // and the first non-whitespace character on the right
      for (let y = lastIdx; y--; ) {
        ...
        if (`\n\r`.includes(str[y]) || str[y].trim()) {
          DEV && console.log(`940 RETURN ${y + 1 + (str[y].trim() ? 1 : 0)}`);
          return y + 1 + (str[y].trim() ? 1 : 0);
        }
      }
    }
    // ELSE
    DEV && console.log(`946 ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`} 0`);
    return 0;
  }
  if (opts.mode === 1) {
    // mode 1 doesn't touch the whitespace, so it's quick:
    DEV && console.log(`951 RETURN ${lastIdx}`);
    return lastIdx;
  }
  if (opts.mode === 2) {
    // mode 2 hungrily chomps all whitespace except newlines
    ...
    if (
      ...
      for (let y = lastIdx; y--; ) {
        if (str[y].trim() || `\n\r`.includes(str[y])) {
          DEV && console.log(`965 RETURN ${y + 1}`);
          return y + 1;
        }
      }
    }
    // ELSE, last but not least, chomp to the end:
    DEV && console.log(`971 RETURN 0`);
    return 0;
  }
  // ELSE - mode 3
  ...
  DEV &&
    console.log(
      `979 RETURN ${whatsOnTheLeft !== null ? whatsOnTheLeft + 1 : 0}`
    );
...
function chompLeft(str: string, idx: number, ...args: any[]): number | null {
  DEV &&
    console.log(
      `1021 chompLeft(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
        args,
        null,
        4
      )}`
  ...
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    DEV && console.log(`1029 return null because there's nothing to match`);
    return null;
  }
  DEV && console.log(`1032 chompLeft()`);
  ...
  if (isObj(args[0])) {
    ...
    DEV && console.log(`1065 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("left", str, idx, opts, clone(args).slice(1));
  }
  if (!isStr(args[0])) {
    DEV &&
      console.log(`1070 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
  ...
  DEV && console.log(`1075 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
...
function chompRight(str: string, idx: number, ...args: any[]): number | null {
  DEV &&
    console.log(
      `1108 chompRight(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
        args,
        null,
        4
      )}`
  ...
  // if there are no arguments, null
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    DEV && console.log(`1116 return null because there's nothing to match`);
    return null;
  }
  DEV && console.log(`1119 chompRight()`);
  ...
  if (isObj(args[0])) {
    ...
    DEV && console.log(`1148 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("right", str, idx, opts, clone(args).slice(1));
  }
  if (!isStr(args[0])) {
    DEV &&
      console.log(`1153 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
    return chomp("right", str, idx, defaults, clone(args).slice(1));
  }
  // ELSE
  // all arguments are values to match, first element is not options object
  DEV && console.log(`1158 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
