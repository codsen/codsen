...
function seq(direction, str, idx, opts, args) {
  ...
  while (i < args.length) {
    ...
    if (!isStr(args[i]) || !args[i].length) {
      console.log(
        `218 continue because ${JSON.stringify(
          args[i],
          null,
          4
    ...
    console.log(
      `234 ${`\u001b[${33}m${`value`}\u001b[${39}m`} = ${JSON.stringify(
        value,
        null,
        4
    ...
    if (
      ...
      console.log(
        `256 SET whattsOnTheSide = ${whattsOnTheSide} (${str[whattsOnTheSide]})`
      );
      ...
      if (direction === "right" && whattsOnTheSide > lastFinding + 1) {
        console.log(`283 push gap [${lastFinding + 1}, ${whattsOnTheSide}]`);
        gaps.push([lastFinding + 1, whattsOnTheSide]);
      } else if (direction === "left" && whattsOnTheSide < lastFinding - 1) {
        console.log(`286 unshift gap [${whattsOnTheSide + 1}, ${lastFinding}]`);
        gaps.unshift([whattsOnTheSide + 1, lastFinding]);
      }
      console.log(`289 ${`\u001b[${32}m${value} MATCHED!\u001b[${39}m`}`);
      ...
      console.log(`307 SET lastFinding = ${lastFinding}`);
    } else if (optional) {
      console.log(
        `310 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because it was optional`
      );
      i++;
      continue;
    } else if (satiated) {
      console.log(
        `316 ${`\u001b[${32}m${`CONTINUE`}\u001b[${39}m`} because "satiated" is on`
      );
      i++;
      satiated = undefined;
      continue;
    } else {
      console.log(`322 RETURN null`);
      return null;
    }
  }
  console.log(
    `327 ${`\u001b[${34}m${`███████████████████████████████████████ seq() stops looping`}\u001b[${39}m`}`
  );
  console.log(`329 FINAL gaps = ${JSON.stringify(gaps, null, 4)}`);

  // if all arguments in sequence were empty strings, we return falsey null:
  if (leftmostChar === undefined) {
    console.log(`333 RETURN ${`\u001b[${33}m${`null`}\u001b[${39}m`}`);
    return null;
  }

  console.log(
    `338 RETURN ${`\u001b[${33}m${JSON.stringify(
      {
        gaps,
        leftmostChar,
        rightmostChar
...
function leftSeq(str, idx, ...args) {
  ...
  if (!args.length) {
    console.log(`379 leftSeq() calling left()`);
    return left(str, idx);
  }
  const defaults = {
    i: false
  ...
  console.log(
    `392 leftSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
      opts,
      null,
      4
    )}`
  ...
  console.log(`399 leftSeq() calling seq()`);
...
function rightSeq(str, idx, ...args) {
  ...
  if (!args.length) {
    console.log(`406 rightSeq() calling right()`);
  ...
  console.log(
    `419 rightSeq() ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
      opts,
      null,
      4
    )}`
  );
  console.log(`425 rightSeq() calling seq()`);
...
function chomp(direction, str, idx, opts, args) {
  ...
  if (
    ...
    console.log(`473 there's no space to go further in this direction`);
    return null;
  }
  ...
  console.log(
    `482 ${`\u001b[${33}m${`opts`}\u001b[${39}m`} = ${JSON.stringify(
      opts,
      null,
      4
  ...
  do {
    ...
    // console.log(
    //   `501 ${`\u001b[${36}m${`██`}\u001b[${39}m`} initial lastRes = ${JSON.stringify(
    //     direction === "right"
    ...
    console.log(
      `518 ${`\u001b[${36}m${`lastRes = ${JSON.stringify(
        lastRes,
        null,
        4
      )}`}\u001b[${39}m`}`
    ...
    if (lastRes !== null) {
      ...
      console.log(
        `528 ${`\u001b[${36}m${`another sequence; confirmed! Now set `}\u001b[${39}m`} ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${JSON.stringify(
          lastIdx,
          null,
          4
        )};`
  ...
  console.log(`543 ${`\u001b[${33}m${`lastIdx`}\u001b[${39}m`} = ${lastIdx}`);
  ...
  if (direction === "right") {
    ...
    if (str[lastIdx] && str[lastIdx].trim().length) {
      ...
      console.log(`565 RETURN ${lastIdx}`);
      return lastIdx;
    }
    ...
    console.log(
      `572 SET ${`\u001b[${33}m${`whatsOnTheRight`}\u001b[${39}m`} = ${whatsOnTheRight}`
    );
    if (opts.mode === 0) {
      if (whatsOnTheRight === lastIdx + 1) {
        ...
        for (let y = lastIdx, len = str.length; y < len; y++) {
          if (`\n\r`.includes(str[y])) {
            console.log(`592 RETURN ${y}`);
            return y;
          }
        }
      } else {
        console.log(
          `598 ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`} ${
      ...
      if (
        ...
        for (let y = lastIdx, len = str.length; y < len; y++) {
          if (str[y].trim().length || `\n\r`.includes(str[y])) {
            console.log(`626 RETURN ${y}`);
            return y;
          }
        }
      }
      // ELSE, last but not least, chomp to the end:
      console.log(`632 RETURN ${str.length}`);
      return str.length;
    }
    // ELSE - mode 3

    // mode 3 is an aggro chomp - will chump all whitespace
    console.log(`638 RETURN ${whatsOnTheRight ? whatsOnTheRight : str.length}`);
  ...
  if (str[lastIdx] && str[lastIdx - 1] && str[lastIdx - 1].trim().length) {
    ...
    console.log(`660 RETURN ${lastIdx}`);
    return lastIdx;
  }
  ...
  console.log(
    `668 SET ${`\u001b[${33}m${`whatsOnTheLeft`}\u001b[${39}m`} = ${whatsOnTheLeft}`
  );
  if (opts.mode === 0) {
    if (whatsOnTheLeft === lastIdx - 2) {
      ...
      // the final result
      console.log(`674 ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`} ${lastIdx}`);
      return lastIdx;
    } else if (
      ...
      console.log(
        `682 ${`\u001b[${36}m${`loop backwards from ${lastIdx}`}\u001b[${39}m`}`
      );
      // if there are line break characters between current "lastIdx" we're on
      // and the first non-whitespace character on the right
      for (let y = lastIdx; y--; ) {
        console.log(
          `688 ${`\u001b[${36}m${`str[${y}] = ${JSON.stringify(
            str[y],
            null,
            0
          )}`}\u001b[${39}m`}`
        ...
        if (`\n\r`.includes(str[y]) || str[y].trim().length) {
          console.log(`695 RETURN ${y + 1 + (str[y].trim().length ? 1 : 0)}`);
          return y + 1 + (str[y].trim().length ? 1 : 0);
        }
      }
    }
    // ELSE
    console.log(`701 ${`\u001b[${32}m${`RETURN`}\u001b[${39}m`} 0`);
    return 0;
  } else if (opts.mode === 1) {
    // mode 1 doesn't touch the whitespace, so it's quick:
    console.log(`705 RETURN ${lastIdx}`);
    return lastIdx;
  } else if (opts.mode === 2) {
    // mode 2 hungrily chomps all whitespace except newlines
    ...
    if (
      ...
      for (let y = lastIdx; y--; ) {
        if (str[y].trim().length || `\n\r`.includes(str[y])) {
          console.log(`718 RETURN ${y + 1}`);
          return y + 1;
        }
      }
    }
    // ELSE, last but not least, chomp to the end:
    console.log(`724 RETURN 0`);
    return 0;
  }
  // ELSE - mode 3

  // mode 3 is an aggro chomp - will chump all whitespace
  console.log(`730 RETURN ${whatsOnTheLeft !== null ? whatsOnTheLeft + 1 : 0}`);
...
function chompLeft(str, idx, ...args) {
  console.log(
    `770 chompLeft(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
      args,
      null,
      4
    )}`
  ...
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    console.log(`778 return null because there's nothing to match`);
    return null;
  }
  console.log(`781 chompLeft()`);
  ...
  if (isObj(args[0])) {
    ...
    console.log(`810 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("left", str, idx, opts, clone(args).slice(1));
  } else if (!isStr(args[0])) {
    console.log(`813 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
  ...
  console.log(`818 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
...
function chompRight(str, idx, ...args) {
  console.log(
    `850 chompRight(): received ${`\u001b[${33}m${`args`}\u001b[${39}m`} = ${JSON.stringify(
      args,
      null,
      4
    )}`
  ...
  // if there are no arguments, null
  if (!args.length || (args.length === 1 && isObj(args[0]))) {
    console.log(`858 return null because there's nothing to match`);
    return null;
  }
  console.log(`861 chompRight()`);
  ...
  if (isObj(args[0])) {
    ...
    console.log(`890 FINAL opts = ${JSON.stringify(opts, null, 4)}`);
    return chomp("right", str, idx, opts, clone(args).slice(1));
  } else if (!isStr(args[0])) {
    console.log(`893 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
    return chomp("right", str, idx, defaults, clone(args).slice(1));
  }
  // ELSE
  // all arguments are values to match, first element is not options object
  console.log(`898 FINAL opts = ${JSON.stringify(defaults, null, 4)}`);
