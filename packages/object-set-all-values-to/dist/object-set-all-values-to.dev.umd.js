...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.objectSetAllValuesTo = factory());
  ...
  function setAllValuesTo(inputOriginal, valueOriginal) {
    ...
    if (arguments.length < 2) {
      value = false;
    } else if (lodash_isplainobject(valueOriginal) || isArr(valueOriginal)) {
    ...
    if (isArr(input)) {
      input.forEach(function (el, i) {
        if (lodash_isplainobject(input[i]) || isArr(input[i])) {
          input[i] = setAllValuesTo(input[i], value);
        }
      });
    } else if (lodash_isplainobject(input)) {
      Object.keys(input).forEach(function (key) {
        if (isArr(input[key]) || lodash_isplainobject(input[key])) {
