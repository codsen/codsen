...
import { auditDevConsoleLogLineLabels } from "../helpers/devConsoleLogLineLabels.js";
...
for (const filename of packageSources) {
  ...
  const result = auditDevConsoleLogLineLabels(
...
if (problems.length) {
  console.error(
    `DEV console.log line-label verification failed with ${problems.length} problem${problems.length === 1 ? "" : "s"}:\n${problems
      .map(
  ...
  console.log(
    `DEV console.log line labels OK: ${checkedCount} logs across ${packageSources.length} TypeScript source files`,
  );
}
